"""Architecture evidence for the private Xiaohongshu mapping seam."""
...
_PACKAGE = _SRC / "modules/xiaohongshu_adapter"
_SKILL = _PACKAGE / "application/skills/xiaohongshu_brief_mapping"
_OUTPUT = _SKILL / "output_contract.py"
...
def _consumer_violations(path: Path, tree: ast.Module) -> list[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            if all(
                alias.name == "ai_ecommerce_agent.modules.xiaohongshu_adapter.public"
                for alias in node.names
            ):
                continue
            if any("xiaohongshu_brief_mapping" in alias.name for alias in node.names):
        ...
        allowed_public_import = (
            node.level == 0
            and (
                imported == "ai_ecommerce_agent.modules.xiaohongshu_adapter.public"
                or (
                    imported == "ai_ecommerce_agent.modules.xiaohongshu_adapter"
                    and aliases == {"public"}
                )
            )
            and _PUBLIC.is_file()
        ...
        allowed_pipeline_import = (
            ...
            and imported
            == "ai_ecommerce_agent.modules.xiaohongshu_adapter.application.skills"
            and aliases == {"xiaohongshu_brief_mapping"}
        )
        if allowed_pipeline_import:
            continue
        if (
            "xiaohongshu_brief_mapping" in imported
            or "xiaohongshu_brief_mapping" in aliases
            or _OUTPUT in targets
        ):
...
def _import_time_effects(tree: ast.Module) -> tuple[list[ast.Call], list[ast.expr]]:
    ...
    def arguments(values: ast.arguments) -> None:
        for argument in (*values.posonlyargs, *values.args, *values.kwonlyargs):
...
def test_exact_new_file_inventory_and_path_imports() -> None:
...
def test_repository_has_only_xiaohongshu_facade_as_mapping_consumer() -> None:
    violations: list[str] = []
...
def test_consumer_guard_allows_public_and_facade_only() -> None:
    facade = _SKILL / "__init__.py"
    assert not _consumer_violations(
        facade,
        ast.parse(
            "from .output_contract import xiaohongshu_brief_candidate_output_spec\n"
        ),
    )
    assert not _consumer_violations(
    ...
    for source in (
        ...
        "import ai_ecommerce_agent.modules.xiaohongshu_adapter.public as public\n",
    ):
        assert not _consumer_violations(public_consumer, ast.parse(source))
    sibling = _SKILL / "sibling.py"
    for source in (
        "from .output_contract import xiaohongshu_brief_candidate_output_spec\n",
        "from . import output_contract\n",
        (
            "from ai_ecommerce_agent.modules.xiaohongshu_adapter.application.skills."
            "xiaohongshu_brief_mapping import xiaohongshu_brief_candidate_output_spec\n"
        ),
    ):
