"""One explicit Qwen Token Plan supplemental Task-to-export smoke.

The module is skipped unless both the Qwen supplemental opt-in and the
existing local PostgreSQL opt-in are set.  The adapter owns Secret resolution
and fails before PostgreSQL setup when the operator has not injected a key.
The Secret name and value are intentionally absent from this test module.
"""
...
_RUN_QWEN = os.environ.get("RUN_QWEN_SUPPLEMENTAL_SMOKE") == "1"
_RUN_POSTGRES = os.environ.get("MVP0_RUN_TASK_HTTP_POSTGRES") == "1"
if _RUN_QWEN and not os.environ.get("GIT_COMMIT", "").strip():
    pytest.fail(
        "RUN_QWEN_SUPPLEMENTAL_SMOKE=1 requires GIT_COMMIT for evidence; "
        "value is not displayed"
    )
if _RUN_QWEN and not os.environ.get("FL2_QWEN_LIVE_EVIDENCE_PATH", "").strip():
    pytest.fail(
        "RUN_QWEN_SUPPLEMENTAL_SMOKE=1 requires an operator-selected evidence path"
    )
if not (_RUN_QWEN and _RUN_POSTGRES):
    pytest.skip(
        "set MVP0_RUN_TASK_HTTP_POSTGRES=1 and RUN_QWEN_SUPPLEMENTAL_SMOKE=1 for "
        "the single Qwen supplemental smoke",
        allow_module_level=True,
    )

BACKEND_ROOT = Path(__file__).resolve().parents[2]
SCHEMA = "mvp0_fl2_qwen_live"
...
def live_runtime_preflight() -> Iterator[None]:
    """Let the private adapter validate operator configuration first."""
...
def _write_evidence(
    *,
    ...
    write_live_smoke_evidence(
        Path(os.environ["FL2_QWEN_LIVE_EVIDENCE_PATH"].strip()),
        serialized,
    )


def test_one_qwen_supplemental_task_to_export_smoke(postgres_engine: Engine) -> None:
    """Run exactly one sufficient Anchor SKU path with five Qwen calls."""
    ...
    try:
            ...
            confirmed = client.post(
                ...
                json={
                    ...
                    "marketingCoreMessage": (
                        "Confirmed Qwen supplemental storage message"
                    ),
                    "xiaohongshuTitleDirection": (
                        "Confirmed Qwen supplemental title direction"
                    ),
                },
            )
        ...
        _write_evidence(
            ...
            disposition="PASS",
            reason=(
                "Qwen supplemental automated gates passed; operator must record "
                "human result"
            ),
            runtimes=runtimes,
        ...
        _write_evidence(
            ...
            started_clock=started_clock,
            disposition="FAIL",
            reason="Qwen supplemental automated smoke failed; operator review required",
            runtimes=runtimes,
            behavior_gates=gates,
        )
        raise
