...
- **当前阶段：** [MVP-0L Local AI Web App Delivery Goal](docs/goals/mvp0-local-ai-web-app-delivery-goal.md) — L0 governance activation is `ACTIVE` after PR #317 reached `main`; L1 is the active [Issue #318](https://github.com/JettxonHo/ai-ecommerce-agent/issues/318) Real Needs Input backend Stage. Its independent five-axis review is `PASS`; the replacement PR and merge remain pending, and L1 becomes merge-effective only after that replacement PR reaches `main`. L2 remains gated. The predecessor [MVP-0P Local Action Workbench Productization Goal](docs/goals/mvp0-local-action-workbench-productization-goal.md) is historical `MVP0P_GOAL_COMPLETE`. The frozen successor order is L0 → L1 → L2 → L3 → L4 → L5 → L6 under [DEC-084](docs/decisions/dec-084-apple-silicon-local-ai-web-app-goal.md).
- **开发状态：** the predecessor final review merged in PR #315, so `MVP0P_GOAL_COMPLETE` is current historical truth. Issue #318's additive PostgreSQL Needs Input implementation, one-page Web recovery consumer and one-time provider-free runtime acceptance are present on this clean-history replacement branch; independent five-axis review is `PASS`, while the replacement PR and merge remain pending. L1 is not merge-effective until the replacement PR reaches `main`, and L2 remains gated. The historical [MVP-0 Fast Lane Goal](docs/goals/mvp0-fast-lane-goal.md) remains terminal `GOAL_BLOCKED`, preserving two failed DeepSeek runs, [DEC-081](docs/decisions/dec-081-mvp0-fl2-offline-diagnosis-and-bounded-repair.md) `INSUFFICIENT_SANITIZED_EVIDENCE` and observational ambiguity, no Provider acceptance, no production repair / Phase B contract and no inherited live authorization. P5 remains `P5_REUSE_FROZEN`; Spider_XHS reuse and platform behavior are frozen and unauthorized.
- **最小目标：** deliver the existing deterministic Action Workbench toward a real-AI local Web App for Apple Silicon Macs only, with Docker Desktop installed by the user and the system default browser as the product surface. The eventual real-AI contract is official DeepSeek `deepseek-v4-pro`; L0 was docs-only, and the active L1 implementation remains provider-free. Ordinary Git/GitHub documentation workflow transport remains allowed.
- **当前事实：** the repository has Task/Source persistence, fixed-workspace routes, generated client, Workbench projection, deterministic scripted pipeline and local demo path. Issue #318 adds the real PostgreSQL-backed Needs Input read/resolve boundary, current-request projection, bounded recovery reconciliation and the existing Web one-page Intake consumer without changing OpenAPI/generated types. Acceptance uses fictional/sanitized data only. Native macOS App/WebView, signing/notarization, login/RBAC/multi-user/public deployment and Keychain/Secret UI are Deferred. Intel support is Deferred; excluded from the first release. The later project-root Git-ignored `.env` convention is accepted, but no Stage may create/read/inspect it or any Secret.
- **当前 Gate：** L0 is merged/current and L1 Issue #318 has a recorded independent five-axis review `PASS` after its exact provider-free runtime acceptance; the replacement PR and merge remain pending, L1 is not merge-effective until that replacement PR reaches `main`, and L2 remains gated. No Provider/model/live call, Secret access, public-contract/generated-client change, dependency/lockfile change, unrelated migration/schema expansion, native App work, public deployment or Spider_XHS action is authorized. Full access for ordinary reversible local work does not override independent review, Secret/provider/platform gates, destructive-action controls, public-contract/migration gates or the exact implementation-agent rule.
