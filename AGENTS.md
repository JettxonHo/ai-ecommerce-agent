...
- **当前阶段：** MVP-0 Fast Lane FL-2 DeepSeek Provider Amendment and Real-provider Proof
- **开发状态：** ACTIVE — FL-1 deterministic browser-to-backend loop 与 FL-3 one-command local demo 已完成；PR #266 合并了离线 Qwen Token Plan adapter，但其 live 因 Provider Terms 冻结。用户已通过 [DEC-079](docs/decisions/dec-079-deepseek-v4-pro-mvp0-real-provider-amendment.md) 将剩余 FL-2 Gate 改为 direct DeepSeek official `deepseek-v4-pro`；Issue #268 先完成治理文档，随后才创建独立实现 Issue。旧 [MVP-0 Goal](docs/goals/end-to-end-demo-mvp0-goal.md) 的已完成工作保留，剩余横向 Backlog 不再自动执行。
...
- **当前事实：** 仓库已有 Task/Source 持久化、Durable Dispatch 与 Model Runtime seams、五类 output contracts、安全 Markdown renderer、FastAPI fixed-workspace business routes、authored OpenAPI、generated client、Task gateway、`/tasks` list/create/read routes、Task-scoped input/result/review/export、Workbench projection、deterministic scripted pipeline 与 real-backend Chromium coverage；`scripts/mvp0/demo` 提供固定 PostgreSQL + API + Vite 的前台本地启动路径。OpenAI / Qwen / DeepSeek 均没有形成 live acceptance；PR #266 的 Qwen code 仅代表离线实现。
- **当前 Gate：** 先合并 DEC-079 / Goal / current-status 的 documentation amendment，再由准确名称的 `luna-worker` 实现 DeepSeek private adapter 与 opt-in smoke seam。真实 Provider 调用仍需 exact head、Required Checks、Sol 五轴 Review 和用户单独付费授权；`DEEPSEEK_API_KEY` 不得进入聊天、GitHub、source、logs、CI 或 evidence。Issue #255 在本修订合并后被取代；Issue #264 记录 `BLOCKED_BY_PROVIDER_TERMS` 后关闭。JSON/CSV/PDF/图片、完整 Retrieval/Evidence、分布式 Worker/Checkpoint Recovery、高级 Review/Diff、完整 API catalog、Auth/Multi-tenant/Public Deployment 均后移。现有非阻塞复杂代码冻结而非提前清理。
