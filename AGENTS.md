...
- **最小目标：** deliver the existing deterministic Action Workbench toward a real-AI local Web App for Apple Silicon Macs only, with Docker Desktop installed by the user and the system default browser as the product surface. The eventual real-AI contract is official DeepSeek `deepseek-v4-pro`; L0 is docs-only and performs no runtime or external action.
- **当前事实：** the repository already has Task/Source persistence, fixed-workspace routes, generated client, Workbench projection, deterministic scripted pipeline and local demo path. The current FastAPI task resource still projects `needsInputRequest: null` and does not implement a real Needs Input read/resolve resource; this is the explicit L1 starting gap. Acceptance uses fictional/sanitized data only. Native macOS App/WebView, signing/notarization, Intel, login/RBAC/multi-user/public deployment and Keychain/Secret UI are Deferred. The later project-root Git-ignored `.env` convention is accepted, but L0 must not create/read/inspect it or any Secret.
