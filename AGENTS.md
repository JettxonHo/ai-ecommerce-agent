...
- **当前阶段：** [MVP-0L Local AI Web App Delivery Goal](docs/goals/mvp0-local-ai-web-app-delivery-goal.md) — L0 governance activation is `ACTIVATION_PENDING` on the Issue #316 branch and becomes `ACTIVE` only when its PR reaches `main`. The predecessor [MVP-0P Local Action Workbench Productization Goal](docs/goals/mvp0-local-action-workbench-productization-goal.md) is historical `MVP0P_GOAL_COMPLETE`. The frozen successor order is L0 → L1 → L2 → L3 → L4 → L5 → L6 under [DEC-084](docs/decisions/dec-084-apple-silicon-local-ai-web-app-goal.md).
- **开发状态：** the predecessor final review merged in PR #315 at `main@c5dc2ae9d6e56c6da35f823df838cab4518830f0`, so `MVP0P_GOAL_COMPLETE` is current historical truth. The successor remains branch-pending: no L1 Issue or implementation Stage is active until the L0 PR reaches `main`. The historical [MVP-0 Fast Lane Goal](docs/goals/mvp0-fast-lane-goal.md) remains terminal `GOAL_BLOCKED`, preserving two failed DeepSeek runs, [DEC-081](docs/decisions/dec-081-mvp0-fl2-offline-diagnosis-and-bounded-repair.md) `INSUFFICIENT_SANITIZED_EVIDENCE` and observational ambiguity, no Provider acceptance, no production repair / Phase B contract and no inherited live authorization. P5 remains `P5_REUSE_FROZEN`; Spider_XHS reuse and platform behavior are frozen and unauthorized.
- **最小目标：** deliver the existing deterministic Action Workbench toward a real-AI local Web App for Apple Silicon Macs only, with Docker Desktop installed by the user and the system default browser as the product surface. The eventual real-AI contract is official DeepSeek `deepseek-v4-pro`; L0 is docs-only and performs no product-runtime/Provider/API/platform action. Ordinary Git/GitHub documentation workflow transport remains allowed.
- **当前事实：** the repository already has Task/Source persistence, fixed-workspace routes, generated client, Workbench projection, deterministic scripted pipeline and local demo path. The current FastAPI task resource still projects `needsInputRequest: null` and does not implement a real Needs Input read/resolve resource; this is the explicit L1 starting gap. Acceptance uses fictional/sanitized data only. Native macOS App/WebView, signing/notarization, login/RBAC/multi-user/public deployment and Keychain/Secret UI are Deferred. Intel support is Deferred; excluded from the first release. The later project-root Git-ignored `.env` convention is accepted, but L0 must not create/read/inspect it or any Secret.
- **当前 Gate：** L0 is a docs-only activation gate with an exact nine-path allowlist. No code, test, configuration, dependency, lockfile, migration, OpenAPI/generated client, Web implementation, Docker/API/PostgreSQL launch, environment/Secret access, Provider/model/live call, Kimi/Terra action, native App work, public deployment or Spider_XHS action is authorized. Full access for ordinary reversible local work does not override independent review, Secret/provider/platform gates, destructive-action controls, public-contract/migration gates or the exact implementation-agent rule.
