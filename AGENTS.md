...
- **当前阶段：** [MVP-0L Local AI Web App Delivery Goal](docs/goals/mvp0-local-ai-web-app-delivery-goal.md) — L0 governance activation is `ACTIVE` after PR #317 reached `main`; L1 [Issue #318](https://github.com/JettxonHo/ai-ecommerce-agent/issues/318) is merged/current through PR #328 after its independent five-axis review and provider-free runtime acceptance. L2 [Issue #329](https://github.com/JettxonHo/ai-ecommerce-agent/issues/329) is the active minimum persistence acceptance/reconciliation Stage; its provider-free runtime and independent five-axis review are `PASS` at exact code/test head `2c1c1d39c44b77803f587785f07f741f8374ef29`, with fresh Required Checks `12/12 successful`. PR #330 remains `OPEN`/unmerged during this docs reconciliation, so L2 is not merge-effective until reviewed closure reaches `main`, and L3 remains gated. The predecessor [MVP-0P Local Action Workbench Productization Goal](docs/goals/mvp0-local-action-workbench-productization-goal.md) is historical `MVP0P_GOAL_COMPLETE`. The frozen successor order is L0 → L1 → L2 → L3 → L4 → L5 → L6 under [DEC-084](docs/decisions/dec-084-apple-silicon-local-ai-web-app-goal.md).
- **开发状态：** the predecessor final review merged in PR #315, so `MVP0P_GOAL_COMPLETE` is current historical truth. Issue #318's additive PostgreSQL Needs Input implementation, one-page Web recovery consumer and one-time provider-free runtime acceptance are merge-effective through PR #328. Issue #329 adds only a bounded integration characterization and current-truth reconciliation; its six-test follow-up runtime passed once in `1.41s` on an isolated ephemeral scope, and the blocking stale revision/idempotency finding was resolved by test-only assertions at code/test head `2c1c1d3`. Independent five-axis review is `PASS`, fresh Required Checks are `12/12`, and PR #330 remains open/unmerged; L2 is not merge-effective until reviewed closure reaches `main`, and L3 remains gated. The owner-confirmed MBL-first order remains L2 → L3 → L4 → L5 → Agent UI → L6. The historical [MVP-0 Fast Lane Goal](docs/goals/mvp0-fast-lane-goal.md) remains terminal `GOAL_BLOCKED`, preserving two failed DeepSeek runs, [DEC-081](docs/decisions/dec-081-mvp0-fl2-offline-diagnosis-and-bounded-repair.md) `INSUFFICIENT_SANITIZED_EVIDENCE` and observational ambiguity, no Provider acceptance, no production repair / Phase B contract and no inherited live authorization. P5 remains `P5_REUSE_FROZEN`; Spider_XHS reuse and platform behavior are frozen and unauthorized.
- **最小目标：** deliver the existing deterministic Action Workbench toward a real-AI local Web App for Apple Silicon Macs only, with Docker Desktop installed by the user and the system default browser as the product surface. The eventual real-AI contract is official DeepSeek `deepseek-v4-pro`; L0 was docs-only, L1 is merged/current, and L2's provider-free persistence characterization/review is complete at code/test head `2c1c1d3` while PR #330 remains the merge gate. Ordinary Git/GitHub documentation workflow transport remains allowed.
...
- **当前 Gate：** L0 and L1 are merged/current through PRs #317 and #328. L2 Issue #329 has provider-free runtime `PASS`, independent five-axis `PASS` and fresh Required Checks `12/12` at exact code/test head `2c1c1d39c44b77803f587785f07f741f8374ef29`; PR #330 is still `OPEN`/unmerged, so L2 is not merge-effective until reviewed closure reaches `main`, and L3 remains gated. No Provider/model/live call, Secret access, public-contract/generated-client change, migration/schema expansion, dependency/lockfile change beyond the single offline Web hydration with byte-identity proof, native App work, public deployment or Spider_XHS action is authorized. Full access for ordinary reversible local work does not override independent review, Secret/provider/platform gates, destructive-action controls, public-contract/migration gates or the exact implementation-agent rule.
