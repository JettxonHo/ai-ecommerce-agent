...
- **当前阶段：** [DEC-087](docs/decisions/dec-087-mvp0l-terminal-rebaseline-and-pilot-activation.md) 已在 `main` 生效：MVP-0L 为 `TERMINAL_INCOMPLETE_L5_FAILED`、Real Product-to-Brief Pilot 为 `ACTIVE`。Issue #341 的 P0 admission/contract-freeze record 将在本 PR 合并前保持 P01–P08 `ADMISSION_PENDING_REVIEW`、denominator `FREEZE_PENDING_REVIEW`、P0 `CONTRACT_FREEZE_PENDING`；该 record 出现在 `main` 后才使 P01–P08 `ADMITTED`、denominator exact 8 frozen、P0 `P0_CONTRACT_FROZEN`、P1 `READY_NOT_STARTED` 生效。
...
- **当前 Gate：** Issue #341 is the single P0 admission/contract-freeze Issue. Its exact seven-path docs-only PR must preserve the accepted DEC-086/087 and Pilot Contract semantics, add no eighth path, and must not claim a run, numerator or Goal completion before merge. No runtime, Provider/model call, Secret access, public-contract/generated-client change, migration/schema expansion, dependency/lockfile change, native App, public deployment or Spider_XHS action is authorized.
- **Pilot Gate：** [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md), its [Pilot Contract](docs/product/real-product-to-brief-pilot-contract.md), and the [P0 admission/contract-freeze plan](docs/product/real-product-to-brief-pilot-p0-plan.md) retain the exact **P0 → P1 → P2 → P3 → P4 → P5 → P6** order and accepted denominator. The merge-durable P0 state is `CONTRACT_FREEZE_PENDING` before this PR and `P0_CONTRACT_FROZEN` only after it reaches `main`; P1 remains `READY_NOT_STARTED` then. Future P1 carries the observed post-confirm/no-export boundary as a provider-free characterization target, not an approved repair. Every Provider/cohort execution still requires a new exact-commit owner Gate.
