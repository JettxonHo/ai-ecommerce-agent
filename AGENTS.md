...
- **开发状态：** ACTIVE / `GOAL_BLOCKED` — FL-1 deterministic browser-to-backend loop 与 one-command local demo 已完成；[PR #271](https://github.com/JettxonHo/ai-ecommerce-agent/pull/271) 已按 [DEC-079](docs/decisions/dec-079-deepseek-v4-pro-mvp0-real-provider-amendment.md) 合并 direct DeepSeek official `deepseek-v4-pro` private adapter、项目 Schema / bounded Domain admission 与 opt-in smoke seam。2026-08-13 在 exact reviewed `main@1c7c2107ead332235d492ed063b67101784d35f1` 执行的唯一一次已授权 smoke 完成了一个虚构 Task 的五次调用且零重试，但在进入 `awaiting_review` 前安全失败；不得声明 live verified。旧 [MVP-0 Goal](docs/goals/end-to-end-demo-mvp0-goal.md) 的已完成工作保留，剩余横向 Backlog 不再自动执行。
