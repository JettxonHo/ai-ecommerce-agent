...
- **当前阶段：** [DEC-087](docs/decisions/dec-087-mvp0l-terminal-rebaseline-and-pilot-activation.md) 的 docs-only rebaseline 由 [Issue #339](https://github.com/JettxonHo/ai-ecommerce-agent/issues/339) / [PR #340](https://github.com/JettxonHo/ai-ecommerce-agent/pull/340) 记录。Before PR #340 reaches `main`，branch statuses are MVP-0L `REBASELINE_PENDING`、Real Product-to-Brief Pilot `ACTIVATION_PENDING`、P0 `NOT_STARTED`。When this record is present on `main` after PR #340 merges，merge-effective statuses are MVP-0L `TERMINAL_INCOMPLETE_L5_FAILED`、Pilot `ACTIVE`、P0 `READY_NOT_STARTED`。
...
- **最小目标：** preserve the working deterministic/local foundation and truthfully transition from the incomplete MVP-0L release attempt to the bounded Real Product-to-Brief business Pilot. Issue #339 performs documentation only: no MVP0L repair, L6, Agent UI, P0 execution, cohort data, real inputs, Provider/Secret action or runtime.
...
- **当前 Gate：** Issue #339 must reach `main` before the rebaseline is effective. Until then the branch must retain `REBASELINE_PENDING` / `ACTIVATION_PENDING` / `NOT_STARTED` and must not claim P0 execution or Goal completion. No runtime, Provider/model call, Secret access, public-contract/generated-client change, migration/schema expansion, dependency/lockfile change, native App, public deployment or Spider_XHS action is authorized.
- **Pilot Gate：** [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md) and its [Pilot Contract](docs/product/real-product-to-brief-pilot-contract.md) retain the exact **P0 → P1 → P2 → P3 → P4 → P5 → P6** order and accepted denominator. After this PR reaches `main`, Pilot is `ACTIVE` and P0 is only `READY_NOT_STARTED`. Future P1 carries the observed post-confirm/no-export boundary as a provider-free characterization target, not an approved repair. Every Provider/cohort execution still requires a new exact-commit owner Gate.
