...
- **当前阶段：** [MVP-0L Local AI Web App Delivery Goal](docs/goals/mvp0-local-ai-web-app-delivery-goal.md) — L0 governance activation is `ACTIVE` after PR #317 reached `main`; L1 [Issue #318](https://github.com/JettxonHo/ai-ecommerce-agent/issues/318) is merged/current through PR #328 after its independent five-axis review and provider-free runtime acceptance. L2 [Issue #329](https://github.com/JettxonHo/ai-ecommerce-agent/issues/329) / PR #330 is merged/current at `origin/main@dbccacacc54cb21c393987a8612dfc6aa825093b` with provider-free runtime, independent five-axis review and fresh Required Checks `12/12`; Issue #329 is closed and L2 is merge-effective. L3 [Issue #331](https://github.com/JettxonHo/ai-ecommerce-agent/issues/331) / PR #332 is merged/current at `origin/main@2124a9bb20d6b7b327c828331bdc8293ec76577e` after offline GREEN and one corrected-pin provider-free runtime `PASS` (historical first build-only `HOLD` on unpublished `uv==0.12.8` preserved), independent five-axis review `PASS` at `f831519`, and fresh Required Checks `12/12`. L4 [Issue #333](https://github.com/JettxonHo/ai-ecommerce-agent/issues/333) is **closed** via merged PR #334 at `origin/main@ef5ac0d4b372c1478ee2541ee3ec5318e72a1060`, with disposition `L4_OFFLINE_QUALIFIED_NO_JUSTIFIED_REPAIR`, production diff zero and no Phase-B amendment. L5 [Issue #335](https://github.com/JettxonHo/ai-ecommerce-agent/issues/335) / [PR #336](https://github.com/JettxonHo/ai-ecommerce-agent/pull/336) is terminal `L5_REAL_AI_ACCEPTANCE_FAIL_NO_EXPORTS` at exact head `2210d68ad6e09e1a402dd63b0cd9b2a52cdfe74f`; its one-time authorization is consumed and no further run is authorized. The predecessor [MVP-0P Local Action Workbench Productization Goal](docs/goals/mvp0-local-action-workbench-productization-goal.md) is historical `MVP0P_GOAL_COMPLETE`. The frozen successor order is L0 → L1 → L2 → L3 → L4 → L5 → L6 under [DEC-084](docs/decisions/dec-084-apple-silicon-local-ai-web-app-goal.md) and [DEC-085](docs/decisions/dec-085-docker-only-local-web-lifecycle.md).
- **开发状态：** the predecessor final review merged in PR #315, so `MVP0P_GOAL_COMPLETE` is current historical truth. Issue #318's additive PostgreSQL Needs Input implementation, one-page Web recovery consumer and one-time provider-free runtime acceptance are merge-effective through PR #328. Issue #329 / PR #330 is merged/current at exact base `dbccacacc54cb21c393987a8612dfc6aa825093b`; its six-test provider-free runtime, independent five-axis review and fresh Required Checks are `PASS`/`12/12`, and Issue #329 is closed. Issue #331 / PR #332 is merged/current at exact base `2124a9bb20d6b7b327c828331bdc8293ec76577e`; its offline RED→GREEN, rendered-Compose evidence, historical build-only `HOLD`, tests-first `uv==0.12.6` repair and single corrected-pin provider-free runtime `PASS` are preserved. L4 Issue #333 is closed via merged PR #334 at exact `origin/main@ef5ac0d4b372c1478ee2541ee3ec5318e72a1060`, with `L4_OFFLINE_QUALIFIED_NO_JUSTIFIED_REPAIR`, no production diff and no Phase-B contract. Current #335 / PR #336 records terminal `L5_REAL_AI_ACCEPTANCE_FAIL_NO_EXPORTS` at exact head `2210d68ad6e09e1a402dd63b0cd9b2a52cdfe74f`; its one-time authorization is consumed and no further run is authorized. The owner-confirmed MBL-first order remains L2 → L3 → L4 → L5 → Agent UI → L6. The historical [MVP-0 Fast Lane Goal](docs/goals/mvp0-fast-lane-goal.md) remains terminal `GOAL_BLOCKED`, preserving two failed DeepSeek runs, [DEC-081](docs/decisions/dec-081-mvp0-fl2-offline-diagnosis-and-bounded-repair.md) `INSUFFICIENT_SANITIZED_EVIDENCE` and observational ambiguity, no Provider acceptance, no production repair / Phase B contract and no inherited live authorization. P5 remains `P5_REUSE_FROZEN`; Spider_XHS reuse and platform behavior are frozen and unauthorized.
- **最小目标：** deliver the existing deterministic Action Workbench toward a real-AI local Web App for Apple Silicon Macs only, with Docker Desktop installed by the user and the system default browser as the product surface. The eventual real-AI contract is official DeepSeek `deepseek-v4-pro`; L0–L3 are merged/current at `origin/main@2124a9bb20d6b7b327c828331bdc8293ec76577e`, and L4 Issue #333 is closed via merged PR #334 at exact `origin/main@ef5ac0d4b372c1478ee2541ee3ec5318e72a1060`, with no production repair. L5 #335 / PR #336 is terminal `L5_REAL_AI_ACCEPTANCE_FAIL_NO_EXPORTS` at exact head `2210d68ad6e09e1a402dd63b0cd9b2a52cdfe74f`; its authorization is consumed and no further run is authorized. Ordinary Git/GitHub documentation workflow transport remains allowed.
...
- **当前 Gate：** L0–L3 are merged/current through PRs #317, #328, #330 and #332; L3 is merge-effective at exact `origin/main@2124a9bb20d6b7b327c828331bdc8293ec76577e`. L4 Issue #333 is **closed** via merged PR #334 at exact `origin/main@ef5ac0d4b372c1478ee2541ee3ec5318e72a1060`, preserving disposition `L4_OFFLINE_QUALIFIED_NO_JUSTIFIED_REPAIR`, production diff zero and no Phase-B amendment. Current L5 is terminal `L5_REAL_AI_ACCEPTANCE_FAIL_NO_EXPORTS` at Issue #335 / PR #336 exact head `2210d68ad6e09e1a402dd63b0cd9b2a52cdfe74f`; its one-time authorization is consumed and no further Provider run is authorized. No additional runtime, Provider/model/live call, Secret access, public-contract/generated-client change, migration/schema expansion, dependency/lockfile change, native App work, public deployment or Spider_XHS action is authorized by G0. Full access for ordinary reversible local work does not override independent review, Secret/provider/platform gates, destructive-action controls, public-contract/migration gates or the exact implementation-agent rule.
- **Pilot Gate：** [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md) and its [Pilot Contract](docs/product/real-product-to-brief-pilot-contract.md) are `ACCEPTED / NOT ACTIVE` under [DEC-086](docs/decisions/dec-086-real-product-to-brief-pilot.md). Activation is blocked until the current MVP-0L Goal is `COMPLETE` or formally rebaselined by the owner. MVP-0L remains the only `ACTIVE` Goal. Current L5 [Issue #335](https://github.com/JettxonHo/ai-ecommerce-agent/issues/335) / [PR #336](https://github.com/JettxonHo/ai-ecommerce-agent/pull/336) records terminal `L5_REAL_AI_ACCEPTANCE_FAIL_NO_EXPORTS` at exact head `2210d68ad6e09e1a402dd63b0cd9b2a52cdfe74f`; its one-time authorization is consumed and no further run is authorized. Any later bounded paid execution or cohort needs fresh exact-commit owner authorization specifying maximum tasks, calls, cost and stop rules; no authorization is inherited. G0 in Issue #337 is docs-only and does not resume it.
