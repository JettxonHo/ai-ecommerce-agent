...
4. **L3 — Local Web lifecycle:** Issue #331 supplies the Apple Silicon + user-installed Docker Desktop one-command lifecycle, bounded preflight/health/stop, private API, loopback Web and system-default-browser ordering. Its offline evidence and one provider-free runtime `PASS` (after the historical first-attempt `HOLD` and corrected pin) are recorded on the implementation branch. Independent five-axis review is `PASS` at `f831519`, fresh Required Checks are `12/12`, and Ready PR #332 is `OPEN`/unmerged; L3 becomes merge-effective/current only when this reviewed record reaches `main`. L4 is gated and not started. Native App/WebView, signing and notarization are Deferred; Intel support is Deferred; excluded from the first release.
...
Read in this order for the active L3 implementation/review:
