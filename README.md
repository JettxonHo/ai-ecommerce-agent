...
> **Branch status:** [MVP-0L Local AI Web App Delivery Goal](docs/goals/mvp0-local-ai-web-app-delivery-goal.md) `REBASELINE_PENDING` · [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md) `ACTIVATION_PENDING` · Pilot P0 `NOT_STARTED`. **Only after this reviewed PR reaches `main`:** MVP-0L `TERMINAL_INCOMPLETE_L5_FAILED` · Pilot `ACTIVE` · P0 `READY_NOT_STARTED`.
>
...
The successor [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md) is `ACTIVATION_PENDING` on this branch under [DEC-087](docs/decisions/dec-087-mvp0l-terminal-rebaseline-and-pilot-activation.md). Only after this PR reaches `main` does it become `ACTIVE`, with P0 only `READY_NOT_STARTED`. Its normative [Pilot Contract](docs/product/real-product-to-brief-pilot-contract.md) preserves **P0 → P1 → P2 → P3 → P4 → P5 → P6**, one active Stage at a time and one Issue per observable outcome. Completion still requires 5–10 permitted products backed by permitted real product material or permitted sanitized real-product material across at least two categories, at least one non-author operator, real Provider evidence, at least one adopted output, three consecutive end-to-end successes without production-code edits, at least 80% approved-export completion, clean/other Apple Silicon evidence, metrics, a sanitized evidence pack and a 2–4 minute demo. P0 fixes the denominator before observation across all admitted product/attempt units, including a failure before export; the numerator is an admitted product/attempt yielding at least one human-approved immutable Marketing or Xiaohongshu export. Issue #339 registers no cohort or numerator and performs no P0 work. Future P1 carries the post-confirm/no-export boundary only as a provider-free characterization target, not an approved repair. Each bounded paid execution or cohort still needs a fresh exact-commit owner authorization.
...
Read in this order for the current docs-only terminal rebaseline:
...
4. [Real Product-to-Brief Pilot Goal](docs/goals/real-product-to-brief-pilot-goal.md) and [Pilot Contract](docs/product/real-product-to-brief-pilot-contract.md)
5. [Session-011](docs/sessions/session-011-mvp0l-terminal-rebaseline.md) and [Implementation Readiness](docs/handoffs/implementation-readiness.md)
