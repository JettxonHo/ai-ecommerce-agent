...
Issue #350 is the current provider-free operator-binder branch. Its
...
explicit review/finalization. On this branch
`OPERATOR_BINDER_IMPLEMENTED = PENDING_INDEPENDENT_REVIEW`; only a reviewed
Ready PR reaching `main` may record `OPERATOR_BINDER_IMPLEMENTED = YES`.
Provider calls, paid calls, Secret reads/injections, Pilot samples,
participant executions and actual charge remain zero; the real P01 Grant is
`NOT_CONSUMED`.
