...
Issue #352 is the current exact-commit, artifact-root and input-handoff
control-alignment branch, pending independent review. Its provider-free
pre-call checks do not authorize a real P01 run. The historical Grant remains
`NOT_CONSUMED` but is `STALE_FOR_NEW_MAIN` after #351; the exact future input
handoff is `/Users/ketchup/Private/ai-ecommerce-pilot/inputs/p01-public.txt`,
and `REAL_P01_INPUT_FILE_READY=NO` is based only on Owner/pre-call authority
without inspecting the private root. Provider calls, paid calls, Secret
reads/injections, Pilot samples, participant executions and actual charge
remain zero; `P01_ATTEMPT_EXECUTED=NO` and `P01_RESULT=NOT_EXECUTED` remain
current. The next action is
