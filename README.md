...
6. preserve the two terminal direct DeepSeek failure records and execute only the DEC-081 Phase A offline diagnosis; production repair waits for independently reviewed evidence and an exact Phase B contract, while any future Provider run requires a new exact-commit contract and user authorization.
...
- DEC-081 Phase A is offline diagnosis only; Phase B requires independent Phase A review and an exact repair contract, and no future Provider call inherits authorization from either phase.
