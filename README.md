...
- RFC-003 LangGraph Runtime（`IN REVIEW`；DEC-049～051 已冻结 DQ-01～09，整体仍待最终一致性 Review 后由用户单独接受）、RFC-004 API / Human Review、RFC-005 Source / Retrieval、RFC-006 LLM Runtime、RFC-007 Observability；
...
**CONDITIONALLY READY（规划范围）** — Foundation 已完成，RFC-003 正在最终 Review，但业务实现仍未授权。必须完成 RFC-003～007、Frontend Architecture、Readiness 规划包、测试策略、Goal 文本和最终一致性 Review，并由用户明确批准“进入 Goal 执行阶段”后，才可开始长期开发。详见 [Implementation Readiness](docs/handoffs/implementation-readiness.md) 与 [AGENTS.md](AGENTS.md)。
