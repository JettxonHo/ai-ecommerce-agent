...
- RFC-005 DQ-01～03 已由 DEC-067 接受：Source / SourceVersion / revisioned TaskSourceAssociation / DerivedArtifact 分离；逐 Source 原子登记与耐久处理；六值 processing lifecycle；格式感知 Fragment / Locator；Evidence Package 不新增 `package_hash`。DQ-04～06 已由 DEC-068 接受：PostgreSQL-native Exact / `tsvector` / `pg_trgm` / `pgvector` derived plane、filtered exact NN、单一版本化 OpenAI Embedding Profile、immutable index generation、确定性 Direct-first Planner、RRF、4 / 20 / 60 / 12 seed bounds，以及首 Goal 无 LLM rewrite / baseline reranker。DQ-07～09 已由 DEC-069 接受：server-derived SQL authorized candidate relation、窄 Source / Evidence 投影与 cursor、immutable RetrievalRun + referenced EvidencePackage、DatasetStatistic / Formal Evidence atomic commit、代表性评测硬门禁与显式降级。DQ-10 / P-67 与 RFC-005 整体仍待接受，Retrieval 实现尚未授权。

---

## 当前策划缺口

当前 Gate 顺序已由用户于 2026-08-07 明确确认为：**产品规格闭合 → RFC-004 → RFC-005 → RFC-007**。Product Specification 与 RFC-004 均已整体接受，PR #55 已合并且 Issue #54 已关闭。当前由 [Issue #56](https://github.com/JettxonHo/ai-ecommerce-agent/issues/56) / [Draft PR #57](https://github.com/JettxonHo/ai-ecommerce-agent/pull/57) 承载 RFC-005 Source Processing and Retrieval Architecture 策划；P-58A～P-66A / DQ-01～09 已由 DEC-067～069 接受，当前只剩 P-67 / DQ-10 与 RFC-005 整体仍为 `PROPOSED`。未接受 Proposal 不得写成实现事实。
