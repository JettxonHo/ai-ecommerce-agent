...
docs-only closure follow-up pending review. Its provider-free pre-call checks
do not authorize a real P01 run. The historical Grant is
`NOT_CONSUMED_BUT_STALE_FOR_NEW_MAIN`; `AUTHORIZATION_STATUS=REQUIRES_NEW_OWNER_GRANT`.
The exact future input handoff is
`/Users/ketchup/Private/ai-ecommerce-pilot/inputs/p01-public.txt`, and
`REAL_P01_INPUT_FILE_READY=NO` is based only on Owner/pre-call authority
