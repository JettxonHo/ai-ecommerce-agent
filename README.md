...
- RFC-005 DQ-01～03 已由 DEC-067 接受：Source / SourceVersion / revisioned TaskSourceAssociation / DerivedArtifact 分离；逐 Source 原子登记与耐久处理；六值 processing lifecycle；表单 / TXT / Markdown / 文本 PDF / 评论 CSV 的格式感知 Fragment / Locator；Evidence Package 使用可读 identity / version，不新增 `package_hash`。RFC-005 整体与 Retrieval 实现尚未授权。

---

## 当前策划缺口

当前 Gate 顺序已由用户于 2026-08-07 明确确认为：**产品规格闭合 → RFC-004 → RFC-005 → RFC-007**。Product Specification 与 RFC-004 均已整体接受，PR #55 已合并且 Issue #54 已关闭。当前由 [Issue #56](https://github.com/JettxonHo/ai-ecommerce-agent/issues/56) / [Draft PR #57](https://github.com/JettxonHo/ai-ecommerce-agent/pull/57) 承载 RFC-005 Source Processing and Retrieval Architecture 策划；P-58A～P-60A / DQ-01～03 已由 DEC-067 接受，当前 P-61～P-63 / DQ-04～06 与 RFC-007 仍为 `PROPOSED`，未接受 Proposal 不得写成实现事实。
