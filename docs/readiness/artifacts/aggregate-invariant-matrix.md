...
>
> **Ownership sync（2026-08-09）：** DEC-076 已关闭 INV-009 / INV-010 的 exact package Gate；DEC-077 已关闭 INV-005～INV-008 的 exact package Gate，并冻结 Review Draft / Decision 与 Export Snapshot ownership。其他仍标记 `PENDING BUSINESS DEFINITION` 的 Aggregate 不受本次接受影响。
...
| INV-005 | Approved Strategy（Approved Strategy Version） | 仅 Approved Strategy Version 进入 Marketing Brief；接受 Hypothesis ≠ Hypothesis→Fact；Evidence Limitations 不得删除 | `modules/human_review/`（DEC-077） | DEC-035 Atomic Business Commit（submit 事务原子更新 Pointer） |
| INV-006 | Review Package（Review Package Version） | 固定上游版本输入快照；上游版本变化 → 标 superseded，旧提交被阻止 | `modules/human_review/`（DEC-077） | DEC-035 Atomic Business Commit（创建与 Interrupt 分离） |
| INV-007 | Marketing Brief（Marketing Brief Version） | Authoritative Input 仅 approved_strategy_version_id；Message/Benefit Hierarchy 与 Proof Point 追溯链成立 | `modules/marketing_brief/`（DEC-077） | DEC-035 Atomic Business Commit |
| INV-008 | Xiaohongshu Execution Brief（Execution Brief Version） | Authoritative Input 仅 marketing_brief_version_id；limitations_or_fit_boundary 不可删 | `modules/xiaohongshu_adapter/`（DEC-077） | DEC-035 Atomic Business Commit |
| INV-009 | Source / Source Version（Raw Information Current Truth） | 业务结果必须引用具体 source_version_id（非 source_id）；source_scope 隔离当前商品/竞品 | `modules/source_evidence/`（DEC-076） | DEC-035 Atomic Business Commit（Source Blob 内容寻址 + 短事务 Finalize，DQ-12） |
| INV-010 | Evidence Link（Fragment↔业务结论已验证关系） | Formal Evidence Link 仅在 Skill 输出过 Evidence Validator 后创建；指向不可变 SourceVersion | `modules/source_evidence/`（DEC-076） | DEC-035 Atomic Business Commit（与业务版本同事务） |
...
| INV-005 | `modules/human_review/` | Approved Strategy immutable content（DomainVersionId + VersionNumber）+ mutable family Version State | `(task_id, approved_strategy)` 内单调版本 | 用户 submit 通过完整 outer transaction | Approved Strategy 整体；Version State 独立 | 承接全部 Positioning Elements + Hypothesis Decisions；Stage pointer 选择 Current Truth |
| INV-006 | `modules/human_review/` | immutable Review Package snapshot（ReviewPackageId + VersionNumber） | 同一 ReviewId 内正数单调版本 | create_review_package | Package 整体 | 固定上游版本；上游变化标 superseded；Draft identity / revision 与 Package basis 分离 |
| INV-007 | `modules/marketing_brief/` | Marketing Brief immutable content（DomainVersionId + VersionNumber）+ mutable family Version State | `(task_id, marketing_brief)` 内单调版本 | 生成 / 用户编辑 | Brief 整体；Version State 独立 | Authoritative Input 仅 approved_strategy_version_id；Stage pointer 选择 Current Truth |
| INV-008 | `modules/xiaohongshu_adapter/` | Xiaohongshu Brief immutable content（DomainVersionId + VersionNumber）+ mutable family Version State | `(task_id, xiaohongshu_brief)` 内单调版本 | 生成 / 用户编辑 | Execution Brief 整体；Version State 独立 | Authoritative Input 仅 marketing_brief_version_id；Stage pointer 选择 Current Truth |
| INV-009 | `modules/source_evidence/` | immutable SourceVersion + revisioned SourceVersionProcessing + revisioned TaskSourceAssociation | 单来源内容快照与独立处理 / membership Current Truth | 来源登记、处理迁移、remove / replace | SourceVersion 整体；processing 与 association 独立记录 | immutable version、processing 与 association 不互相覆盖（DEC-076） |
| INV-010 | `modules/source_evidence/` | Evidence Link（关系对象） | 单条已验证关系 | Evidence Validator 通过 | 单关系对象 | 指向不可变 SourceVersion + 适用 Fragment/Selector |
...
| INV-005 | Task Management-owned Human Review Stage `current_version` / `last_valid_version` + Task status / revision | 完整 submit 的 outer transaction 经 typed Task participant 原子更新；不建平行 pointer table | Withdrawal / upstream invalidation 经 Version State + Stage pointer 显式表达，不改 immutable content | Business Restore（新前向 Command） | exact Review / Package / Draft basis + upstream version refs | Approved Strategy immutable content + family Version State + Task Stage projection（DEC-077） |
...
| INV-007 | Task Management-owned Marketing Brief Stage `current_version` / `last_valid_version` + Task status / revision | 成功 outer commit 经 typed Task participant 提升；不建平行 pointer table | Brief revise 不使上游失效，但使当前 Xiaohongshu Version State / Stage pointer 失效 | Business Restore | approved_strategy_version_id | Marketing Brief immutable content + family Version State + Task Stage projection（DEC-077） |
| INV-008 | Task Management-owned Xiaohongshu Stage `current_version` / `last_valid_version` + Task status / revision | 成功 outer commit 经 typed Task participant 提升；不建平行 pointer table | 当前 MVP 最终输出，普通编辑不触发下游失效 | Business Restore | marketing_brief_version_id | Xiaohongshu Brief immutable content + family Version State + Task Stage projection（DEC-077） |
| INV-009 | exact TaskSourceAssociation identity / revision / membership + exact SourceVersion + SourceVersionProcessing Current Truth | 登记、processing CAS 或 replace 原子事务更新 owning records；replace 创建新 active association identity | association `removed / replaced` 与 processing `superseded` 分维度显式表达，不改 immutable SourceVersion | Replace 必须创建新 SourceVersion / association 且不原地 re-activate；仅 `removed` association 的恢复 identity 仍为 Open Question | historical manifest 固定 association identity / revision / SourceVersion | `modules/source_evidence/` 的 immutable SourceVersion、processing Current Truth 与 association records（DEC-076） |
...
| INV-005 | Strategy/Review capability（DQ-15） | DQ-11 · DEC-029 · DEC-024 · DEC-028 · DEC-077 | data-architecture.md §DEC-029 + DEC-077 | Source Object = ACCEPTED DECISION（DEC-029/077）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（DQ-11/DEC-077）；Owning Module = `modules/human_review/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
| INV-006 | Review capability（DQ-15） | DQ-11 · DEC-029 · DEC-077 | data-architecture.md §DEC-029 + DEC-077 | Source Object = ACCEPTED DECISION（DEC-029/077）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（快照对象，DQ-11/DEC-077）；Owning Module = `modules/human_review/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
| INV-007 | Brief capability（DQ-15） | DQ-11 · DEC-030 · DEC-024 · DEC-029 · DEC-077 | data-architecture.md §DEC-030 + DEC-077 | Source Object = ACCEPTED DECISION（DEC-030/077）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（DQ-11/DEC-077）；Owning Module = `modules/marketing_brief/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
| INV-008 | Adapter capability（DQ-15） | DQ-11 · DEC-031 · DEC-024 · DEC-030 · DEC-077 | data-architecture.md §DEC-031 + DEC-077 | Source Object = ACCEPTED DECISION（DEC-031/077）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（DQ-11/DEC-077）；Owning Module = `modules/xiaohongshu_adapter/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
| INV-009 | Source/Evidence capability（DQ-15） | DQ-11 · DQ-12 · DEC-025 · DEC-076 | data-architecture.md §DEC-025 + DEC-076 · rfc-002-decision-questions.md §DQ-12 | Source Object = ACCEPTED DECISION（DEC-025/076/DQ-12）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（DQ-11/DQ-12/DEC-076）；Owning Module = `modules/source_evidence/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
| INV-010 | Source/Evidence capability（DQ-15） | DQ-12 · DEC-025 · DEC-032 · DEC-076 | data-architecture.md §DEC-025/032 + DEC-076 | Source Object = ACCEPTED DECISION（DEC-025/032/076）；Aggregate Classification = ACCEPTED AS ARP-01 PLANNING BASELINE；Invariant = ACCEPTED WHERE EXPLICITLY SOURCED；Version/Lifecycle = ACCEPTED WHERE EXPLICITLY SOURCED（关系对象）；Owning Module = `modules/source_evidence/`；Artifact Row = ACCEPTED / OPEN QUESTIONS PRESERVED |
...
- [x] 未接受对象的具体 package 名称仍保留 `PENDING BUSINESS DEFINITION`；DEC-076 / DEC-077 已冻结 Source/Evidence 与 Review/Brief/Adapter/Export 接受对象的具体 owner，不声称其生产模块已实现。
...
| # | 问题 | 决定所有者 | 下一 Gate |
|---|---|---|---|
| OQ-1 | 尚未由 DEC-076 / DEC-077 覆盖的 Aggregate 正式 Owning Module package 命名与边界 | 用户 + Persistence/Domain Owner | `PENDING BUSINESS DEFINITION`：仅对未接受对象保留；Source/Evidence 与 Review/Brief/Adapter/Export 接受对象已冻结 owner，具体生产实现仍待 child Issues |
| OQ-2 | Source / Evidence 是否作为独立模块或跨模块能力 | 用户 + Source/Evidence Owner | RESOLVED：DEC-076 冻结 `modules/source_evidence/` 为唯一 owner；不复制 Source / Evidence 事实源。 |
