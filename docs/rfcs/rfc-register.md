...
- **RFC-001 与 RFC-002 已 `ACCEPTED`**（用户正式决定）；RFC-003 当前为 **`DRAFTING`**；RFC-004～RFC-007 当前为 **`PROPOSED`**。各 RFC 的正文与接受与否由用户在相应 Decision Gate 决定。
...
RFC-003 Status = DRAFTING (DEC-049 / DEC-050 accepted inputs DQ-01 through DQ-06 only; RFC acceptance not granted)
