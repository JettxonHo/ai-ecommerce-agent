...
- **RFC-001、RFC-002、RFC-003 与 RFC-006 已 `ACCEPTED`**（用户正式决定）；RFC-004 当前为 `DRAFTING` 且所有本轮 Proposal 未接受，RFC-005 / RFC-007 为 `PROPOSED`。各 RFC 的正文与接受与否由用户在相应 Decision Gate 决定。
...
| RFC-004 | **[API and Human Review Architecture](rfc-004-api-and-human-review-architecture.md)**（生产 API 边界、Human Review 提交/暂停协议、权限） | Wave 3 | P0 | `DRAFTING`（Issue #54；DQ-01～06 Accepted，DQ-07～09 Proposed，DQ-10 Pending） | Review / Orchestration 接口层 | DEC-007/029/044～047/055～057/059～064 · workflow/human-review spec | R-1 |
