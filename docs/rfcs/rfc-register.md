...
> 
> **Current truth（2026-08-08）：** Product Specification 与 RFC-001～006 已整体闭合；[RFC-005](rfc-005-source-processing-and-retrieval-architecture.md) Final Review = PASS。[RFC-007](rfc-007-observability-and-runtime-operations.md) = `DRAFTING`（Issue #58；P-68A / P-69A / P-70A proposed；Pre-acceptance Review = PASS AS PROPOSAL）。快速 MVP-0 Development Plan / Testing Strategy / Goal / Readiness 完整 Draft 已形成，P-71A / P-72A / P-73A proposed；Business / Production Implementation、Spike / Live call 与 Goal 仍未激活。
...
- **RFC-001～RFC-006 已 `ACCEPTED`**（用户正式决定）；RFC-007 为 `DRAFTING`。各 RFC 的正文与接受与否由用户在相应 Decision Gate 决定。
...
| RFC-007 | **[Minimal Observability and Runtime Operations for MVP-0](rfc-007-observability-and-runtime-operations.md)**（结构化日志、correlation、错误引用、Retry / Timeout / Backoff ownership 与本地运维证据） | Wave 4 | P1 | `DRAFTING`（Issue #58；P-68A / P-69A / P-70A proposed；Pre-acceptance Review PASS） | Runtime Operations | DEC-033 / 039 / 070 · RFC-002～006 · runtime/failure-recovery spec | R-1, R-4 |
