...
- **RFC-001、RFC-002 与 RFC-003 已 `ACCEPTED`**（用户正式决定）；RFC-006 当前为 **`DRAFTING`**，其中 P-28～P-30 仍为 `PROPOSED`；RFC-004 / RFC-005 / RFC-007 当前为 **`PROPOSED`**。各 RFC 的正文与接受与否由用户在相应 Decision Gate 决定。
...
| RFC-006 | **[LLM Runtime and Structured Output](rfc-006-llm-runtime-and-structured-output.md)**（生产 LLM Provider、结构化输出、真实模型 Smoke 策略、Secret 注入边界） | Wave 2 | P0 | `DRAFTING`（P-28～P-30 = `PROPOSED`，未接受） | 所有 LLM 驱动 Skill | DEC-011/026/027/028/030 · skills specs | R-2 |
...
RFC-006 Status = DRAFTING (P-28 through P-30 proposed; no user decision)
...
Next Gate: product specification closure, RFC-004 through RFC-007, Frontend Architecture, complete readiness planning package, testing strategy, Goal text, final consistency review, and explicit user approval
```
