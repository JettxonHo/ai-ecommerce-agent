...
**待决（→ RFC-002-DQ-08/DQ-09；DQ-07 并发控制自此 ACCEPTED 2026-08-02）：** ~~乐观并发/CAS/数据库约束/应用锁/task-level 序列化的取舍~~（**已由 DQ-07 Accepted Decision 决定：分层并发控制——乐观 revision 默认 + 命名唯一约束最终防线 + Durable Lease + fencing_token 执行所有权 + SKIP LOCKED 仅限队列式 Claim + 进程内锁仅非权威优化**）；Command ID/Idempotency Key/Attempt ID/Run ID/Review Decision ID/Dispatch ID 的唯一性约束（完整幂等键层级留 DQ-08）；是否首版引入 Transactional Outbox（留 DQ-09）。
