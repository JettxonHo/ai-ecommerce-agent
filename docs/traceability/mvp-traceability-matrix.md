...
| Adapter Secret / Payload allowlist, deterministic model substitute and one manual RC smoke | DEC-054 | [RFC-006](../rfcs/rfc-006-llm-runtime-and-structured-output.md) | spike-03（test-design evidence only） | DEC-054 + Issue #48 / PR #49 | RFC-006（DQ-07～08 accepted input；overall pending） | _(placeholder)_ | Secret boundary + payload-free ledger / telemetry + offline contract layers + opt-in live smoke _(planned)_ |
...
RFC-006 = IN REVIEW (DQ-01 through DQ-08 accepted input via DEC-052 / DEC-053 / DEC-054; final review passed; RFC overall acceptance pending)
