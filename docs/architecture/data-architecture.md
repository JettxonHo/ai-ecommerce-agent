...
> **Current sync（2026-08-07）：** RFC-002～006 已 Accepted。DEC-067～069 冻结 Source graph、Processing、Fragment、PostgreSQL-native derived retrieval、Scope、Evidence 与 evaluation。DEC-070 冻结目标 `text-embedding-3-small` / explicit 1536 / cosine Profile 与公共 catalog；快速 MVP-0 只实施 Direct / Exact / PostgreSQL Lexical，text PDF 与 vector / Semantic / Hybrid 后移 MVP-1，但继续服从同一 SourceSet、eligibility、Validator 与 atomic Evidence commit。RFC-005 Final Review = PASS。正文旧 `draft_version`、Source `task_id`、`package_hash`、raw-score Fusion、broad post-filter 或 mutable Evidence 示例与该 Current sync 冲突时，以最新 Accepted DEC 为准。
> **Persistence ownership sync（2026-08-09）：** DEC-076 冻结 `modules/source_evidence/` 唯一 ownership、replace 创建新 active association identity、有界六值 processing graph，以及 immutable SourceVersion / revisioned SourceVersionProcessing / revisioned TaskSourceAssociation 分离。DEC-077 冻结 `human_review` / `marketing_brief` / `xiaohongshu_adapter` / `export_delivery` ownership、immutable content + mutable Version State，以及 Task Management Stage pointer 唯一 Current Truth；完整 Review outcome 继续等待 Audit / Idempotency / Durable Work Intent 参与同一 outer commit。正文旧 package `PENDING` 或平行 pointer 示例与本 sync 冲突时，以 DEC-076 / DEC-077 为准。
