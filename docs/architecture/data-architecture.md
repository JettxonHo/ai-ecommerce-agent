...
> **Current sync（2026-08-06）：** RFC-002 已 Accepted：Business Current Truth 使用 PostgreSQL + SQLAlchemy 2.x synchronous API + Psycopg 3 synchronous driver + Alembic；SQLite 不作为持久化验收引擎。正文中“数据库尚未选择”的表述是 RFC-002 之前的历史快照。Checkpoint 生产实现仍由 RFC-003 决定，最终公共 Schema 仍待产品规格与 API 契约冻结。
