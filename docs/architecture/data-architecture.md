...
> **Current sync（2026-08-06）：** RFC-002 已 Accepted：Business Current Truth 使用 PostgreSQL + SQLAlchemy 2.x synchronous API + Psycopg 3 synchronous driver + Alembic；SQLite 不作为持久化验收引擎。DEC-049 已冻结同 PostgreSQL Service 下独立 Checkpoint Database / Role / Credential / Pool、同步 `PostgresSaver`、`sync` durability 与 Current-Truth-first Reconciliation；RFC-003 整体仍在 Drafting。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义。最终公共 Schema、状态枚举、Diff 算法和 revision 并发仍待 RFC-004 / 006。正文中“数据库或 Checkpointer 尚未选择”的表述是 RFC-002 / DEC-049 之前的历史快照，旧 `draft_version` 示例由 DEC-046 的 `revision` 语义修订。
