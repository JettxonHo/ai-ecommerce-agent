...
> **Current sync（2026-08-07）：** RFC-002 已 Accepted：Business Current Truth 使用 PostgreSQL + SQLAlchemy 2.x synchronous API + Psycopg 3 synchronous driver + Alembic；SQLite 不作为持久化验收引擎。DEC-049～051 已冻结 Checkpoint、Durable Dispatch、ownership、cancel、compatibility 与 recovery；RFC-003 已整体接受。DEC-046～048、DEC-055～056 与 DEC-063～066 已冻结 Review / Brief / Frontend / RFC-004 公共契约。DEC-067 冻结 Source / SourceVersion / revisioned TaskSourceAssociation / DerivedArtifact、逐资料耐久处理、SourceSet manifest 与格式感知 Fragment / Locator，并移除 Evidence Package `package_hash`。DEC-068 冻结同 PostgreSQL Service 的非权威 derived retrieval plane、Exact + `tsvector` / GIN + bounded `pg_trgm` / GIN + `pgvector` filtered exact NN、单一版本化 Embedding Profile、immutable index generation、eligibility-first switch、deterministic Direct-first Planner、RRF 与 4 / 20 / 60 / 12 seed bounds。DEC-069 冻结 server-derived SQL authorized candidate relation、Task-scoped 窄 Source / Evidence projection、immutable RetrievalRun + referenced EvidencePackage、complete DatasetStatistic、Evidence Validator 后的 Domain Version + Formal Evidence Link + Current Truth + audit 原子提交，以及代表性评测硬门禁与显式降级。RFC-005 只剩 DQ-10 的 exact Embedding Profile / public contract / adoption closure 与整体接受。正文旧 `draft_version`、Source `task_id`、processing 状态、`package_hash`、未选择 Retrieval topology、raw-score Fusion、LLM Query Planning、broad post-filter 或 mutable Evidence 示例与该 Current sync 冲突时，以最新 Accepted DEC 为准。
> **Product data constraint sync（2026-08-07）：** DEC-060 要求 Verified Fact、Documented Claim 与 Proof Point 不得越权升格；DEC-061 要求用户资料默认 Task-scoped，可逆移除 / 替换创建版本变化并排除后续 Current Truth 使用，但不等于物理永久删除。最终公共字段、Task 过滤与 Source / Index 一致性由 RFC-004 / 005 冻结；保留、Hold、物理清理与操作员重置由 ARP-08、RFC-007 和 Development Plan 冻结。
...
- **Strategy Draft Revision（临时工作内容，不属 Current Truth）：** 概念字段 `draft_id` / `review_id` / `revision` / `based_on_candidate_ids[]` / `selected_content` / `user_edits[]` / `merge_sources[]` / `hypothesis_decisions[]` / `proof_point_decisions[]` / `user_notes` / `updated_at` / `status`；每次成功保存递增 revision，陈旧保存 / 提交拒绝；**不属于**业务 Current Truth、**不允许**下游使用、**不允许** Marketing Brief 读取；提交前必须通过 Validator。
...
> 注：本节确认**Human Review 与 Approved Strategy 的数据职责**（Review Package Version + Strategy Draft Revision + Review Decisions + Approved Strategy Version + Current Truth Pointer + Hypothesis Decision + Evidence Limitation Acceptance + Proof Point Decision + Review Audit History + Withdrawal Record；承接 DEC-007/009/012/020/023/024/025/028，并由 DEC-046 修订 revision 与产品语义）；概念规格见 [../specs/workflow/human-review-and-approved-strategy-contract.md](../specs/workflow/human-review-and-approved-strategy-contract.md)。**仍待确认** 最终 Review / Approved Strategy 公共 Schema、Strategy Draft 最终 Schema、字段名、Review Status 最终枚举名、Hypothesis / Proof Point / Evidence Limitation Decision 最终字段、Audit Record / Withdrawal Record 最终 Schema、revision 的传输和数据库并发实现、数据库事务实现与数据库表。本节**不**创建最终数据库表 / Review UI / LangGraph Interrupt 代码 / Resume 代码 / Transaction 代码 / Draft 自动保存代码 / Approved Strategy Service 代码，**不**选择并发控制技术 / Draft 存储方案 / 权限系统 / 多人审批系统。
...
- **Durable Dispatch Ownership（DEC-050，概念）：** Transactional Durable Work Intent 是待执行工作的权威来源；Claim 记录当前 `holder_id` / `lease_expires_at` / 单调 `fencing_token`。Heartbeat、完成、释放和由该 Worker 执行产生的正式业务 Commit 条件校验 Holder + Token；接管后的旧 Worker 不得写 Current Truth。
- **Cancellation / Supersession（DEC-050，概念）：** `cancellation_requested` / `superseded` 是持久化请求态；终态须由当前 Owner 确认无部分提交，或由恢复流程证明旧 Lease 已失效且不存在可提交 Owner。取消当前 Run 不删除先前有效 Domain Version。

> 注：本节确认**Workflow Runtime 的数据职责**（Workflow Run / Skill Run / Node Execution / Execution Attempt / Runtime Error Record / Input Fingerprint / Idempotency Record / Recovery Case / Cancellation Record / Runtime Audit Record / Trace Correlation IDs / Checkpoint Reconciliation Metadata / Durable Dispatch Ownership；承接 DEC-007/009/011/012/013/023/024/025/029/032/033/049/050）。概念 Runtime Spec 见 [../specs/runtime/workflow-runtime-failure-recovery-retry-and-observability.md](../specs/runtime/workflow-runtime-failure-recovery-retry-and-observability.md)（仅概念）。以上均为概念结构，**非最终数据库 Schema**。运行记录与业务 Domain Object 分离，运行记录本身**不**是 Current Truth。**仍待确认** Compatibility、Safe Resume Action Matrix、迁移 / 回滚证据、Retry / Timeout / Backoff / Circuit Breaker 参数、轮询 / 批大小 / Lease / Heartbeat / 并发 / Shutdown 参数、DLQ、Logging / Tracing / Metrics / Alerting、数据保留、最终 Schema / 字段 / 状态 / 错误代码。本节**不**创建正式 Runtime、Checkpointer、Worker、Queue / DLQ、数据库表、API、迁移或业务实现代码。
