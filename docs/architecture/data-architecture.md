...
> **Current sync（2026-08-06）：** RFC-002 已 Accepted：Business Current Truth 使用 PostgreSQL + SQLAlchemy 2.x synchronous API + Psycopg 3 synchronous driver + Alembic；SQLite 不作为持久化验收引擎。DEC-046 已冻结 Review / Brief 产品语义、不可变正式对象、Review Draft revision、Current Truth 与导出快照行为；最终公共 Schema 和 revision 并发实现仍待 RFC-004 / 006。正文中“数据库尚未选择”的表述是 RFC-002 之前的历史快照，旧 `draft_version` 示例由 DEC-046 的 `revision` 语义修订。Checkpoint 生产实现仍由 RFC-003 决定。
