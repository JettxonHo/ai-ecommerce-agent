...
> **Current sync（2026-08-06）：** RFC-002 已 Accepted：Business Current Truth 使用 PostgreSQL + SQLAlchemy 2.x synchronous API + Psycopg 3 synchronous driver + Alembic；SQLite 不作为持久化验收引擎。DEC-049 已冻结同 PostgreSQL Service 下独立 Checkpoint Database / Role / Credential / Pool、同步 `PostgresSaver`、`sync` durability 与 Current-Truth-first Reconciliation；DEC-050 已冻结 Durable Dispatch、Lease / fencing 与协作式取消；DEC-051 已冻结运行时 Compatibility Tuple、确定性恢复动作和前向恢复证据边界。RFC-003 的 DQ-01～09 已闭合，并于 2026-08-06 被用户整体接受。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义；DEC-055 / DEC-056 已冻结前端框架、私有投影、语义组级 Diff 与 revision-safe 保存顺序。最终公共 Schema、状态枚举、Change Set、revision 传输与数据库并发仍待 RFC-004。正文中“数据库或 Checkpointer 尚未选择”的表述是 RFC-002 / RFC-003 之前的历史快照，旧 `draft_version` 示例由 DEC-046 的 `revision` 语义修订。
