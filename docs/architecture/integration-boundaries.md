...
> **Current sync（2026-08-06）：** RFC-001～RFC-003 已 Accepted；API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-049～051 已冻结 Checkpoint、Durable Dispatch、Fenced Ownership、取消、兼容和 Safe Resume。DEC-052 已冻结 Application-owned 窄型同步 Model Runtime Port、`platform/model_runtime` 单一 OpenAI Adapter 边界、Composition Root 注入和 Provider SDK 类型隔离；Structured Output 依次通过 Provider 分类、项目 Schema、确定性 Normalization 与 Skill Domain Validator。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义。正文累积段中的旧 `draft_version` 示例按 `revision` 解释。具体 API 协议、状态 / 错误映射、revision 并发、进度传输、精确运行时版本、Retrieval、RFC-006 DQ-04～08、Observability 与 Frontend 集成仍待后续 Gate。
