...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-046 已冻结 Review / Brief 产品语义、不可变正式对象、Review Draft revision、Current Truth 与导出快照；DEC-047 已冻结证据披露、编辑影响、阶段进度、行动导向恢复与导出确认的产品语义。正文累积段中的旧 `draft_version` 示例按 `revision` 解释。具体 API 协议、状态 / 错误映射、revision 并发、进度传输、Checkpointer、Retrieval、LLM Provider、Observability 与 Frontend 集成仍待 RFC-003～007 / Frontend Architecture。
