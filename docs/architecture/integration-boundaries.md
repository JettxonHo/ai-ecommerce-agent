...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，RFC-003 为 `IN REVIEW`；API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-049 已冻结同 PostgreSQL Service 下独立 Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Business-Current-Truth-first Reconciliation；DEC-050 已冻结 PostgreSQL Durable Work Intent + Poll-and-claim、数据库权威 Lease / Heartbeat / Fencing Token 与协作式取消 / Supersession；DEC-051 已冻结显式 Compatibility Tuple、七动作 Safe Resume Matrix、受控迁移和前向恢复证据边界。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义。正文累积段中的旧 `draft_version` 示例按 `revision` 解释。具体 API 协议、状态 / 错误映射、revision 并发、进度传输、精确运行时版本、Retrieval、LLM Provider、Observability 与 Frontend 集成仍待后续 Gate。
...
> **Status clarification：** 上述累积 Status 的“Checkpointer 仍未确认”是 DEC-049 之前的历史边界；当前生产 Checkpointer 输入以本文件 Current sync 与 DEC-049 为准。RFC-003 的 Dispatch / Worker / Cancellation / Compatibility 等其余范围仍未确认。
...
- **Frontend 提交结构化 Review Decision：** 前端通过业务 API（以 Business Database 为准）读取 Review Package / Candidates / 追溯链，提交结构化 Review Decision（select / edit / merge / reject / request_more_information / save_draft / submit / withdraw + Hypothesis Decisions + Proof Point Decisions + Evidence Limitation Acceptance）；**不**直接读取 Checkpointer；提交必须表达 `review_id` / `package_version` / Review Draft `revision` / `idempotency_key` 的语义，最终字段名由 RFC-004 冻结。
- **Approved Strategy Service 执行 18 步原子提交事务：** submit 由 Approved Strategy Service 作为原子事务处理（锁定 Draft 版本 → 校验 Package 状态 / 上游版本 / 必审项 / Schema / Fact·Insight 引用 / Proof Points / Hypothesis 决策 / Evidence Limitations → 创建 Approved Strategy Version → 更新 Current Truth Pointer → Review→approved → Positioning→valid → Review→valid → Marketing Brief→ready → 写 Audit Record → Commit）；任一步失败不创建版本 / 不更新 Pointer / 不改下游阶段；幂等（重复 key 返回首次结果）、Stale 提交拒绝、并发不静默覆盖较新 Draft。
...
> 注：本节确认**Human Review 与 Approved Strategy 各组件的集成边界**（Positioning 只创建候选 / Review Service 创建 Review Package / LangGraph 负责暂停恢复 / Frontend 提交结构化决策 / Approved Strategy Service 执行事务 / Marketing Brief 只读 Approved Strategy / LLM 不自动提交 / Checkpointer 不替代业务库；承接 DEC-007/011/023/024/025/028）；概念规格见 [../specs/workflow/human-review-and-approved-strategy-contract.md](../specs/workflow/human-review-and-approved-strategy-contract.md)（仅概念）。**仍待确认** Review Service 接口、Approved Strategy Service 接口、Review UI、LangGraph Interrupt Payload、Resume 实现、提交事务实现、并发锁实现、Business Repository 接口、正式 API、审核权限、多人协作审核。本节**不**创建 Review UI / LangGraph Interrupt 代码 / Resume 代码 / 数据库表 / API / 并发锁实现 / Transaction 代码 / Draft 自动保存代码 / Approved Strategy Service 代码，**不**选择前端框架 / 数据库 / 并发控制技术 / API 框架 / 权限系统 / 多人审批系统。（Marketing Brief Generation Skill Contract 已由 DEC-030 在概念层确认，见下节；具体 Brief UI / Prompt / Tone 模板 / 风险词库 / CTA 分类等仍属 NOT READY。）
...
> 来源：[DEC-049 — 采用独立 PostgreSQL Checkpoint 数据库、同步持久性与 Current-Truth-first 对账](../decisions/dec-049-dedicated-postgres-checkpoint-sync-durability-and-current-truth-reconciliation.md)；承载 RFC：[RFC-003](../rfcs/rfc-003-langgraph-runtime-and-checkpoint-architecture.md)（`IN REVIEW`，整体尚未 Accepted）。
...
> 来源：[DEC-050 — 采用 PostgreSQL Durable Dispatch、Fenced Worker Ownership 与协作式取消](../decisions/dec-050-postgres-durable-dispatch-fenced-worker-ownership-and-cooperative-cancellation.md)；承载 RFC：[RFC-003](../rfcs/rfc-003-langgraph-runtime-and-checkpoint-architecture.md)（`IN REVIEW`，整体尚未 Accepted）。
...
> 来源：[DEC-051 — 采用显式运行时兼容、确定性安全恢复与前向恢复证据边界](../decisions/dec-051-explicit-runtime-compatibility-deterministic-safe-resume-and-forward-recovery-evidence.md)；承载 RFC：[RFC-003](../rfcs/rfc-003-langgraph-runtime-and-checkpoint-architecture.md)（`IN REVIEW`，整体尚未 Accepted）。
...
> 本节不固定未经实施证据验证的精确依赖版本，不创建 Compatibility Matrix 实例、转换器、Runtime Registry、数据库或迁移，不执行 Spike，也不接受 RFC-003 整体或激活 Goal。
