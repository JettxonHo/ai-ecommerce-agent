...
> **Current sync（2026-08-07）：** RFC-001～RFC-004 与 RFC-006 已 Accepted；API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-049～054 冻结 Workflow / Model Runtime，DEC-046～048 与 DEC-055～056 冻结产品 / Frontend，DEC-063～066 冻结 RFC-004 HTTP Contract。DEC-067 冻结 Source / SourceVersion / revisioned TaskSourceAssociation / DerivedArtifact、逐资料 Durable Processing、SourceSet manifest 与格式感知 Fragment / Locator；Evidence Package 不再使用 `package_hash`。DEC-068 冻结 PostgreSQL-native derived retrieval plane、同一 SQL authorized candidate relation、filtered exact NN、单一版本化 Embedding Profile、immutable generation、deterministic Direct-first Planner、RRF 与有界候选。DEC-069 冻结 server-derived scope、所有 channel 的 SQL pre-ranking filter、窄 Source / Evidence transport、immutable RetrievalRun + referenced EvidencePackage、DatasetStatistic、Evidence Validator 后的原子 Formal Evidence commit、代表性评测硬门禁与显式降级。Source / Evidence Capability 负责权威 eligibility，RFC-003 Worker 承担耐久处理，Frontend 只消费服务端状态。RFC-005 只剩 DQ-10 exact Embedding Profile / public contract / adoption closure 与整体接受；RFC-007 Observability 仍待闭合。
