...
> **Current sync（2026-08-07）：** RFC-001～RFC-003 与 RFC-006 已 Accepted；API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-049～051 已冻结 Checkpoint、Durable Dispatch、Fenced Ownership、取消、兼容和 Safe Resume。DEC-052～054 与 RFC-006 已冻结窄型同步 Model Runtime Port、单一 OpenAI Adapter 隔离、Structured Output、有界 Recovery、可读 Version Tuple、五个固定 Profile、确定性 Context Assembly、Adapter Secret / Payload Allowlist、同 Port Scripted Substitute 与单次人工 RC Smoke。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义；DEC-055 / DEC-056 已冻结深 TaskWorkbench、前端状态所有权、Typed HTTP Adapter Seam、私有交互投影与 revision-safe 保存顺序。DEC-063～066 已冻结 `/api/v1` Contract-first、窄 Resource + typed Command、语义 revision / Idempotency、耐久 Receipt + Run Monitor、Task / Recovery / Review、不可变 Brief / Export、有限 Problem action、fixed-workspace loopback same-origin transport，以及最终 Operation / Schema / state catalog、兼容、generated-client adoption 与 Contract Tests。RFC-004 DQ-01～10 已闭合且 Final Review = PASS，但 RFC 整体仍待用户接受。正文累积段中的旧 `draft_version` 示例按 `revision` 解释；Pagination / Retrieval、精确运行时版本与 Observability 仍待 RFC-005 / 007 及实施证据。
