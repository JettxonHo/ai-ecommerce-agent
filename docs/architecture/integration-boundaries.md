...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-046 已冻结 Review / Brief 产品语义、不可变正式对象、Review Draft revision、Current Truth 与导出快照行为；正文累积段中的旧 `draft_version` 示例按 `revision` 解释。具体 API 协议、revision 并发实现、Checkpointer、Retrieval、LLM Provider、Observability 与 Frontend 集成仍待 RFC-003～007 / Frontend Architecture。
