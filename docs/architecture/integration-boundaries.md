...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。正文中与之冲突的“数据库 / 进程边界未决定”表述只代表形成时历史状态；具体 API 协议、Checkpointer、Retrieval、LLM Provider、Observability 与 Frontend 集成仍待 RFC-003～007 / Frontend Architecture。
...
- **Frontend 提交结构化 Review Decision：** 前端通过业务 API（以 Business Database 为准）读取 Review Package / Candidates / 追溯链，提交结构化 Review Decision（select / edit / merge / reject / request_more_information / save_draft / submit / withdraw + Hypothesis Decisions + Proof Point Decisions + Evidence Limitation Acceptance）；**不**直接读取 Checkpointer；提交须带 `review_id` / `package_version` / `draft_version` / `idempotency_key`。
...
- **Human Review Resume 必须验证 Review Package：** Resume 携带 `review_id` / `review_package_version` / `draft_version` / `approved_strategy_submission_reference`；Resume 前检查 Review Package 未被 superseded、上游版本仍有效、提交事务已成功、Approved Strategy Current Truth 已存在；须幂等；旧 Review Package 不得通过 Checkpoint 绕过 DEC-029 版本校验。
