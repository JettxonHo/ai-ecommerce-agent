...
> **Current sync（2026-08-06）：** RFC-001～RFC-003 已 Accepted；API / Worker / CLI 进程边界和 PostgreSQL 持久化边界已经确认。DEC-049 已冻结同 PostgreSQL Service 下独立 Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Business-Current-Truth-first Reconciliation；DEC-050 已冻结 PostgreSQL Durable Work Intent + Poll-and-claim、数据库权威 Lease / Heartbeat / Fencing Token 与协作式取消 / Supersession；DEC-051 已冻结显式 Compatibility Tuple、七动作 Safe Resume Matrix、受控迁移和前向恢复证据边界。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义。正文累积段中的旧 `draft_version` 示例按 `revision` 解释。具体 API 协议、状态 / 错误映射、revision 并发、进度传输、精确运行时版本、Retrieval、LLM Provider、Observability 与 Frontend 集成仍待后续 Gate。
...
> **Status clarification：** 上述累积 Status 的“Checkpointer 仍未确认”是 DEC-049 之前的历史边界；当前 Workflow Runtime 与 Checkpoint Architecture 以 RFC-003、DEC-049～051 和本文件 Current sync 为准。
