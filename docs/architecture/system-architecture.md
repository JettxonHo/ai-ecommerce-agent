...
> **Current sync（2026-08-08）：** RFC-001～007 已 Accepted，FND-001～003 已完成；统一 Ecommerce Agent、LangGraph StateGraph、PostgreSQL 持久化、OpenAI Responses、contract-first `/api/v1`、MVP-0 Direct / Exact / Lexical Retrieval 与最小 Runtime Operations 已确定。DEC-074 冻结 FastAPI + Uvicorn Adapter（authored OpenAPI 唯一权威）、Compose PostgreSQL + host processes，以及项目自有同步 poll Worker。正文中“具体框架 / 数据库 / Provider 尚未决定”只代表相关章节形成时的历史状态。
