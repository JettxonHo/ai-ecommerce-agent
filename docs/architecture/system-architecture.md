...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，FND-001～003 已完成；统一 Ecommerce Agent、LangGraph StateGraph 与持久化架构均已确定。DEC-046 已冻结 Review / Brief 产品语义、不可变正式对象、Review Draft revision、Current Truth 与导出快照行为，但不冻结公共 Schema。正文中“具体框架 / 数据库尚未决定”只代表相关章节形成时的历史状态。生产 Checkpointer、API、Retrieval、LLM Runtime、Observability 与 Frontend 仍由 RFC-003～007 / Frontend Architecture 决定。
