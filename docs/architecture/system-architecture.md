...
> **Current sync（2026-08-06）：** RFC-001～RFC-003 已 Accepted，FND-001～003 已完成；统一 Ecommerce Agent、LangGraph StateGraph 与持久化架构均已确定。DEC-049～051 与 RFC-003 已冻结 Checkpoint、Durable Dispatch、Fenced Ownership、取消、兼容和 Safe Resume。DEC-052 已冻结首个 Goal 的 OpenAI Responses API / `gpt-5.6-terra` 单一真实 Provider 基线、Application-owned 窄型同步 Model Runtime Port，以及 Provider-native Strict Output → 项目 Schema → Skill Domain Validator 分层；RFC-006 DQ-04～08 与 RFC 整体仍开放。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义，但不冻结公共 Schema 或前端实现。正文中“具体框架 / 数据库 / Provider 尚未决定”只代表相关章节形成时的历史状态。API、Retrieval、LLM Runtime 剩余边界、Observability 与 Frontend 仍由 RFC-004～007 / Frontend Architecture 决定。
...
- 仍未确认：工作流节点数量与最终公共 Schema、RFC-006 的错误 / 版本 / Profile / Secret / Smoke 边界、向量库 / 检索实现、API 框架与协议、前端、部署方式、Observability。**工作流框架已由 DEC-023 选定为 LangGraph；生产持久化与 Checkpointer 已由 RFC-002 / 003 冻结；Provider / Model / Model Runtime Port / Structured Output Authority 已由 DEC-052 冻结。**
