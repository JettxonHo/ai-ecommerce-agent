...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，RFC-003 为 `IN REVIEW`，FND-001～003 已完成；统一 Ecommerce Agent、LangGraph StateGraph 与持久化架构均已确定。DEC-049 已冻结同 PostgreSQL Service 下的独立 Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Business-Current-Truth-first Reconciliation；DEC-050 已冻结 PostgreSQL Durable Work Intent + Poll-and-claim、数据库权威 Lease / Heartbeat / Fencing Token 与协作式取消 / Supersession；DEC-051 已冻结显式 Compatibility Tuple、Current-Truth-first 七动作 Recovery Decision、受控迁移和 Forward Repair 证据边界。RFC-003 的 DQ-01～09 已闭合，但整体尚未 Accepted。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义，但不冻结公共 Schema 或前端实现。正文中“具体框架 / 数据库尚未决定”只代表相关章节形成时的历史状态。API、Retrieval、LLM Runtime、Observability 与 Frontend 仍由 RFC-004～007 / Frontend Architecture 决定。
...
- 仍未确认：工作流节点数量、Skill 与节点最终对应、Workflow State 最终 Schema、State 技术（TypedDict / dataclass / Pydantic）、Reducer 规则、Checkpointer 类型、数据库 / 任务队列、模型（数量 / 是否分模型）、LLM Gateway、向量库 / 检索实现、Worker 实现框架、API / Python 后端框架、前端、部署方式、Observability / LangSmith、基底仓库。**工作流框架本身已由 DEC-023 选定为 LangGraph（StateGraph / Graph API）**；OpenAI Agents SDK 不作主运行时、Temporal 不进 MVP、自研状态机为降级方案。
