...
> **Current sync（2026-08-06）：** RFC-001 / RFC-002 已 Accepted，RFC-003 正在 Drafting，FND-001～003 已完成；统一 Ecommerce Agent、LangGraph StateGraph 与持久化架构均已确定。DEC-049 已冻结同 PostgreSQL Service 下的独立 Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Business-Current-Truth-first Reconciliation；RFC-003 的 Worker / Dispatch / Cancellation / Compatibility 等其余问题仍开放。DEC-046～048 已冻结 Review / Brief / 交互 / 验收 / Markdown 导出产品语义，但不冻结公共 Schema 或前端实现。正文中“具体框架 / 数据库尚未决定”只代表相关章节形成时的历史状态。API、Retrieval、LLM Runtime、Observability 与 Frontend 仍由 RFC-004～007 / Frontend Architecture 决定。
