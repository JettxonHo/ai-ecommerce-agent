...
- **已决定的 RFC-003 输入：** DEC-049 已冻结同 PostgreSQL Service 下的独立 Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Business-Current-Truth-first Reconciliation；DEC-050 已冻结 PostgreSQL Durable Work Intent + Poll-and-claim、数据库权威 Lease / Heartbeat / Fencing Token 与协作式取消 / Supersession；RFC-003 整体仍为 `DRAFTING`。
- **仍未决定：** RFC-003 的 Compatibility、Safe Resume Action Matrix、迁移 / 回滚与验收证据；RFC-004～007、Frontend Architecture、公共 API 细节、Retrieval Backend、LLM Provider / Model、Observability 产品与部署方式。
