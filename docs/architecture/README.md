...
- **已确认：** Modular Monolith、Python 后端、LangGraph StateGraph 确定性编排、版本化 Domain State 与紧凑 Workflow State、Source / Fragment / Evidence Link、按需混合检索、单一 Human Review、API / Worker / CLI 进程边界、模块依赖规则、PostgreSQL + SQLAlchemy 2.x Sync + Psycopg 3 Sync + Alembic 持久化栈，以及 DEC-055 的 React / Vite SPA、显式前端状态所有权、OpenAPI 类型生成和适度浏览器验证基础。
...
- **仍未决定：** 精确实施版本与最终公共字段；Frontend Architecture P-39～P-41、RFC-004 / 005 / 007、公共 API 细节、Retrieval Backend、Observability 产品与部署方式。
...
- [frontend-architecture.md](frontend-architecture.md) — 已接受的前端应用、状态所有权、生成契约和验证基础，以及仍待 P-39～P-41 决定的边界
