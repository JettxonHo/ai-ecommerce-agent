...
- **已确认：** Modular Monolith、Python 后端、LangGraph StateGraph 确定性编排、版本化 Domain State 与紧凑 Workflow State、Source / Fragment / Evidence Link、按需混合检索、单一 Human Review、API / Worker / CLI 进程边界、模块依赖规则、PostgreSQL + SQLAlchemy 2.x Sync + Psycopg 3 Sync + Alembic 持久化栈，以及 DEC-055～056 的 React / Vite SPA 边界。DEC-067 冻结 revisioned TaskSourceAssociation、逐资料 Durable Processing、SourceSet manifest、格式感知 Fragment / Locator 与无 `package_hash` 的重现方式；DEC-068 冻结 PostgreSQL-native derived retrieval plane、filtered exact NN、单一版本化 Embedding Profile、immutable index generation、deterministic Direct-first Planner、RRF 与有界候选；DEC-069 冻结 server-derived authorized candidate relation、窄 Source / Evidence 公共投影、引用式 EvidencePackage、Formal Evidence 原子提交、代表性评测硬门禁与显式降级。
...
- **仍待闭合：** RFC-004 已整体接受；RFC-005 DQ-01～09 已由 DEC-067～069 接受，只剩 P-67 / DQ-10、Final Review 与 RFC-005 整体接受；RFC-007、Observability、部署方式与精确实施版本仍待完成。Frontend Architecture 已整体接受。
