...
- **已完成：** RFC-001～007、Spike-001 与 FND-001～003。
...
- **MVP-0 运行基线：** DEC-073 接受最小诊断、关联、Retry owner 与本地 Release Evidence；DEC-074 接受 FastAPI + Uvicorn（authored OpenAPI 唯一权威）、Compose PostgreSQL + host processes 和项目自有同步 poll Worker。
- 业务与生产实现已在 Active Goal / Issue 合同范围内授权；概念架构 Accepted 仍不等于对应生产模块已经实现。
