...
轮询只针对 DEC-063 接受的窄 Run Resource，并在活动状态结束、Needs Input、waiting Review、manual recovery 或终态停止；DEC-064 / DEC-065 已冻结 Task / Needs Input / Review / Brief / Export 与 Problem action 边界，精确公共状态值和 Operation catalog 仍由 RFC-004 DQ-10 闭合。Stage / 终态变化后刷新 Task Overview 与受影响 Resource，再派生私有投影，前端不得发明业务终态。
