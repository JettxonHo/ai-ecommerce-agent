...
> **Status: ACTIVE PRE-DEVELOPMENT BASELINE（已同步至 DEC-050、RFC-001 / RFC-002；RFC-003 正在 Drafting，RFC-004～007 仍待决定）**
...
- 任何尚未通过 RFC + Accepted Decision 收敛的生产技术（RFC-003 剩余 Compatibility / Safe Resume / 验收证据、API、Retrieval、Observability、部署平台等）在本文件中标记为 **`PENDING RFC`**，**不得**由 Coding Agent 临场选择。DEC-049 / DEC-050 已接受的 Runtime 输入不是 RFC-003 整体 Acceptance，也不授权实现。
...
- **Durable Dispatch / Ownership / Cancellation（DEC-050）**：PostgreSQL Durable Work Intent + 短事务 Poll-and-claim；数据库权威 Lease / Heartbeat / 单调 Fencing Token；持久化协作式取消 / Supersession + Commit Fence。Compatibility、Safe Resume Matrix 与验收证据仍待 RFC-003。
