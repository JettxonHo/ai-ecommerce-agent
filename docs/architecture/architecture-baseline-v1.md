...
> **Status: ACTIVE PRE-DEVELOPMENT BASELINE（已同步至 DEC-053、RFC-001～RFC-003；RFC-006 DQ-01～06 已接受输入，DQ-07～08 与 RFC-004 / 005 / 007 仍待决定）**
...
> **Current sync（2026-08-06）：** RFC-002 已选定 PostgreSQL + SQLAlchemy 2.x Sync + Psycopg 3 Sync + Alembic；DEC-049～051 与 RFC-003 已冻结 Checkpoint、Durable Dispatch、Fenced Ownership、兼容与 Safe Resume。DEC-052 / 053 已冻结单一 OpenAI Responses / `gpt-5.6-terra`、窄型同步 Port、Structured Output 分层、有界 Recovery、可读 Version Tuple、五个固定 Profile 与确定性 Context Assembly；RFC-006 DQ-07～08 与整体仍待决定。RFC-003 的 DQ-01～09 已闭合，并于 2026-08-06 被用户整体接受；FND-001～003 已完成。正文中仍标为 `PENDING RFC` 的数据库、Checkpointer、Worker 或 Foundation 状态是历史快照，以最新 Accepted Decision、RFC 与本说明为准。
...
> 生产后端语言、数据库、ORM 与迁移方案已由 RFC-001 / RFC-002 确认，生产 Checkpointer 已由 RFC-003 确认；LLM Provider / Model / Port / Structured Output / Recovery / Version / Profile 已由 DEC-052 / 053 确认，但 RFC-006 DQ-07～08 与 RFC 整体仍为 `PENDING DECISION`。Retrieval、Observability、Frontend 与部署边界仍为 `PENDING RFC / Decision`（见 [../rfcs/rfc-register.md](../rfcs/rfc-register.md)）。
...
- LLM Provider / Model / Port / Structured Output / Recovery / Version / Profile：ACCEPTED INPUT（DEC-052 / 053）；RFC-006 DQ-07～08 与整体接受仍待决定；
...
| LLM Runtime and Structured Output | PARTIAL ACCEPTED INPUT — DEC-052 / 053；DQ-07～08 / RFC overall pending | RFC-006 |
...
> 本节保留 Foundation 实施期间的收口快照。2026-08-06 当前状态为：RFC-001～003 与 FND-001～003 已完成；RFC-006 DQ-01～06 已由 DEC-052 / 053 接受，DQ-07～08 与 RFC 整体仍在策划；RFC-004 / 005 / 007、Frontend Architecture、产品最终规格、完整 Readiness Artifact、测试与 Goal 文档仍待策划；TS-01～TS-05 执行、业务实现和实际 Goal 均未授权。
