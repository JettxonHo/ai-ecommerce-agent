...
M6: NOT STARTED — API, generated client, Review/Brief persistence and HTTP verticals remain absent
M7: PARTIAL — MVP0-036 Web Foundation COMPLETE (PR #104, merge `adcc38f`); MVP0-037～042 not instantiated / not implemented
M8: NOT STARTED
Active implementation Issues: #81 (Source tracking parent), #82 (Review / Brief / Export tracking parent); #190 is OPEN but implementation-blocked pending direct authorization for `0006` migration and completion-result participant
...
Current blockers: P-87A/P-88A/P-89A remain Proposed for the Source registration boundary; #81 registration/intake, submitted-input metadata and final RFC-005/API/parser/retrieval/evidence slices are not implemented; #82 P-82A～P-86A remain Proposed/pending; Issue #190 requires fresh direct authorization for forward-only `0006` migration and completion-result participant; profile calibration and full Skill work are evidence/dependency blocked
Next: continue only bounded child Issues after the relevant Source proposals and Issue #190 migration gate are explicitly authorized; then build complete Workflow, Skills, API, Review and Web verticals from the merged seams without treating them as already implemented
```
