...
- Production Package 当前包含 Foundation、已完成的 Task Management Task / Run / Stage Domain / Persistence vertical slice（shared values、UoW / PostgreSQL adapter、Alembic `0002_task_management`、application commands 与 CAS）、以及截至 PR #155 Source implementation baseline（merge `cd6bd02fc09a6698d4991dda671131ce03217bcf`）的 Source persistence slices：#109～#113 的 catalogs/snapshots、SourceVersion/processing 与 association domain、typed ports、单一 `0003_source_evidence` head，#121/#124/#126 的 SQLAlchemy Core tables/mappings、PostgreSQL repositories 与 specialized UoW/factory，#128/#130 的 processing application contracts/service/CAS composition，#151/#153 的 Source-owned association remove/replace contracts/service（Task ownership、add-successor-before-old-CAS、rollback/no-orphan evidence），以及 #155 的 immutable no-commit SourceVersion/processing 与 Task-owned association reads；另有 MVP0-036 交付的 React/Vite no-API Web foundation shell。Source registration/intake 与 exact submitted-input metadata、最终 RFC-005 Source HTTP/wire DTOs 或 API/generated client、parser/fragment/retrieval/evidence、Worker、Provider、LangGraph/Model/Retrieval Runtime、业务 Skills 与 Web 业务页面仍由后续有界 Issues 负责；`0003_source_evidence` 的 minimal identity schema 不是 final intake schema。#81 仍为 OPEN tracking parent，M2 仍为 PARTIAL；#82 仍为 OPEN，P-82A～P-86A 保持 Proposed/pending，Review lifecycle/schema 与完整 outcomes 尚未实现。
...
M1 的 5 个 ID 已完成，M2 的 6 个 ID 已实例化为 GitHub Issues（见 §13）；其中 MVP0-006～009（Task Management vertical slice）已完成，MVP0-010 / #81 的 Source persistence slices 已推进至 PR #155 合并的 immutable no-commit reads，#81 仍为 OPEN tracking parent，registration/intake、submitted-input metadata 与最终 RFC-005/API/parser/retrieval boundary 继续按后续有界 Issue 推进。MVP0-011 / #82 仍为 OPEN tracking parent；其 P-82A～P-86A lifecycle proposals 尚未被接受，Review lifecycle/schema 与 complete outcomes 继续等待 gate 和 typed transaction participants。MVP0-036 已完成 M7 foundation slice（PR #104，merge `adcc38f`），MVP0-037～042 尚未实例化或实现；其余 M3～M6、M8 能力与 M7 业务 slices 仍是 Goal 内候选。Sol 在依赖满足时按实际代码状态逐个创建，不得重复创建 M1 / M2 Issues，或把后续候选能力写成已实现。
...
M2 Domain & Persistence: PARTIAL — Task Management vertical slice COMPLETE (#77 / #78 / #79; tracking parent #80 and seven child PRs); Source persistence slices through PR #155 COMPLETE for the merged scope (#109～#113 foundation, #121/#124/#126 tables/mappings/repositories/UoW, #128/#130 processing CAS composition, #151/#153 association remove/replace participant, #155 immutable no-commit reads; PR #155 Source implementation baseline merge `cd6bd02fc09a6698d4991dda671131ce03217bcf`, one Alembic head `0003_source_evidence`), while #81 remains OPEN for registration/intake and submitted-input metadata plus final RFC-005/API/parser/retrieval/evidence boundaries; #82 remains OPEN with P-82A～P-86A Proposed/pending and Review lifecycle/schema and complete outcomes dependency-blocked
M3–M6: NOT STARTED
...
Current blockers: no remaining user Decision Gate for P-74A～P-81A; #81 registration/intake and submitted-input metadata plus final RFC-005/API/parser/retrieval/evidence slices are not implemented; #82 P-82A～P-86A remain Proposed/pending, so Review lifecycle/schema and complete outcomes remain dependency-blocked by explicit acceptance and typed Audit / Idempotency / Durable Work Intent participants
Next: continue the remaining bounded #81 Source slices with exact `luna-worker`, then advance #82 only after the P-82A～P-86A lifecycle gate, remaining #81 prerequisites, and typed transaction-participant prerequisites are met
```
...
| MVP0-010 | [#81](https://github.com/JettxonHo/ai-ecommerce-agent/issues/81) | OPEN tracking parent — Source persistence slices through PR #155 Source implementation baseline (merge `cd6bd02fc09a6698d4991dda671131ce03217bcf`): #109～#113 foundation, #121/#124/#126 tables/mappings/repositories/UoW, #128/#130 processing CAS composition, #151/#153 association remove/replace participant, #155 immutable no-commit reads; registration/intake, submitted-input metadata and final RFC-005/API/parser/retrieval/evidence remain |
| MVP0-011 | [#82](https://github.com/JettxonHo/ai-ecommerce-agent/issues/82) | OPEN tracking parent — P-78A～P-81A accepted by DEC-077; P-82A～P-86A remain Proposed/pending, so Review lifecycle/schema and complete outcomes still wait for explicit acceptance and typed transaction participants |
