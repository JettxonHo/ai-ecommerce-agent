...
>
> **Accepted bounded repair on 2026-08-13:** [DEC-080](../decisions/dec-080-fl2-xiaohongshu-profile-v2-and-deadline-fence.md) authorizes an offline-only `xiaohongshu_mapping_v1 / v2` repair at 16,384 `max_tokens` / 240 s plus a post-return application deadline fence. The other four profiles and all provider/security/retry boundaries remain unchanged. Goal status stays `GOAL_BLOCKED`; neither the Decision nor its implementation authorizes a second paid run. [Issue #274](https://github.com/JettxonHo/ai-ecommerce-agent/issues/274) Phase B remains paused until the repair implementation is independently reviewed and merged.
