...
> **Status: PROPOSED FINAL FOR MVP-0 — NOT ACTIVE**
...
本文件定义首个本地端到端演示 MVP 的测试与验收策略。它固化已接受的产品验收基线、RFC-002～006 的运行与契约边界，以及 DEC-055～056 的前端工具、交互与 Web 质量要求；下列物理布局、命令和证据格式作为快速 MVP-0 策划包的一部分等待用户整体接受。本文不授权业务实现或提前声称 Fixture / Suite 已经存在。
