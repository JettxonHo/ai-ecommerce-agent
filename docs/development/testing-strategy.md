...
- 每 Source 原子登记与处理测试覆盖合法兄弟项不因失败项回滚、六值 processing lifecycle、CSV 合法行 / 有界 row issues，以及 TXT / Markdown / 文本 PDF 不制造伪部分成功；
- 四条 format-aware lane 各使用代表性 Locator case，验证 Fragment 不跨 Source Version / Record / PDF page，原文展示与 normalized search text 分离；不扩展 OCR、任意办公格式或低概率字符矩阵；
- PostgreSQL Integration 使用同一 authoritative candidate relation 验证 Exact、`tsvector` / GIN、bounded `pg_trgm` / GIN 与 `pgvector` filtered exact NN 的 scope / eligibility；应用层 post-filter 不作为通过路径；
- immutable index generation 测试覆盖 expected / present / missing / extra reconciliation、部分 generation 不切换、原子 current-generation switch、remove / replace / restriction 立即从 eligibility 排除，以及历史 Retrieval Run 保留 generation / profile reference；
- deterministic Planner / Fusion 测试覆盖 Direct-first strategy、exact identifier 逐字保留、最多 4 query variants、每通道 20 candidates、RRF constant 60、最多 12 fused candidates、stable Fragment dedup、semantic fallback 与 zero-result `insufficient_information`；这些是行为边界，不计算 Evidence strength 或机械质量总分；
...
- 固定 Retrieval evaluation 复用三个 Anchor SKU 资料变体和 mutation，覆盖 exact、CJK lexical、semantic / hybrid / counter-evidence、scope isolation、complete statistic、zero result、remove / replace、semantic outage 与 incomplete generation；Scope / stale generation / Top-K extrapolation / fabricated zero-result / exact identifier / deterministic order / atomic Formal Evidence 是硬门禁，Relevance 使用 Recall@K / reciprocal-rank / coverage 加人工 `PASS / FAIL`，不合成机械总分；
