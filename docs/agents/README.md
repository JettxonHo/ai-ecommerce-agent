...
> 注：以上为 Skill **定义层**原则（Question-007 已在定义层解决）；**未**确认 Skill 数量、哪些候选 Skill 进入 MVP、Skill 目录、Skill Specification 最终模板、Skill 代码接口、注册与发现、运行时动态选择、Prompt 管理方式、Schema 技术、版本机制、测试框架、Anthropic Skills / OpenAI Skills / MCP / LangGraph / Agent 数量 / Multi-Agent / GitHub 仓库。通用 Skill Spec 结构见 [skill-spec-template.md](skill-spec-template.md)。
