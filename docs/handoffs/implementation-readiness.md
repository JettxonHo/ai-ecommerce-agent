...
>
> **Current Gate（2026-08-09）：** MVP-0 Goal 已激活并由用户明确恢复执行；M1 Issues #63～#67 与 PR #69 / #70 / #71 / #72 / #74 已全部闭合，TS-01 / TS-03 stop-first evidence = PASS。M2 的 Task Management vertical slice 已完成：#77 / #78 / #79 与 tracking parent #80 已关闭，#80 的七个子 Issue 已由 PR #92 / #93 / #94 / #96 / #97 / #99 / #100 合并交付（其基础 PR #84 / #85 / #86 亦已合并）。Source persistence slices through PR #155 are merged at the PR #155 Source implementation baseline (merge `cd6bd02fc09a6698d4991dda671131ce03217bcf`): #109～#113 catalogs/snapshots, SourceVersion/processing and association domains, typed ports and single-head `0003_source_evidence`; #121/#124/#126 SQLAlchemy Core tables/mappings, PostgreSQL repositories and specialized UoW/factory; #128/#130 processing application contracts/service/CAS composition; #151/#153 Source-owned association remove/replace contracts/service with Task ownership and atomic add-successor-before-old-CAS evidence; and #155 additive immutable no-commit SourceVersion/processing and Task-owned association reads. #81（Source）与 #82（Review / Brief / Export）仍保持 OPEN tracking parents，M2 仍为 PARTIAL。#81 registration/intake and exact submitted-input metadata, final RFC-005 Source HTTP/wire DTOs or API/generated client, parser/fragment/retrieval/evidence, Worker, Provider and Web business UI remain unimplemented；`0003_source_evidence` 仍是 minimal identity/processing/association schema，不是 final intake schema。DEC-076 / DEC-077 的 P-74A～P-81A ownership boundaries 已接受；#82 的 P-82A～P-86A 仍为 Proposed/pending，Review lifecycle/schema 与 complete outcomes 仍未实现并等待明确接受及 typed transaction participants。M7 当前为 PARTIAL：MVP0-036 Web Foundation 已由 PR #104 合并（merge `adcc38f`），业务路由、generated client、API-backed Workbench 与 MVP0-037～042 仍未实现。高风险事项仍按 DEC-072 暂停并请求人工确认。
...
- **Workflow Runtime:** 独立 PostgreSQL Checkpoint Database、同步 `PostgresSaver`、`sync` durability、可重入 Node 与 Current-Truth-first Reconciliation = ACCEPTED（DEC-049 / RFC-003）；PostgreSQL Work Intent + Poll-and-claim、数据库权威 Lease / Heartbeat / Fencing Token、协作式 Cancellation / Supersession + Commit Fence = ACCEPTED（DEC-050 / RFC-003）；显式 Compatibility Tuple、Current-Truth-first 七动作 Recovery Decision、受控迁移、Forward Repair 与风险切片证据边界 = ACCEPTED（DEC-051 / RFC-003）。M1 已用 PostgreSQL 16.14、TS-01 与 `langgraph-checkpoint-postgres` 3.1.0 / Store Migration v9 完成 bounded compatibility evidence；M2 Task Management vertical slice 已完成其 Task / Run / Stage migration、adapter、application 与 CAS 证据，Source persistence slices through PR #155 也已交付，但这些均不替代仍未开始的 Source registration/intake metadata、final RFC-005/API/parser/retrieval/evidence、Review / Brief persistence、M4 Runtime 或后续 API / Web 实现，最终公共字段与运维参数继续受 RFC-004 / 007 约束
- **Source / Evidence foundation:** Source persistence slices through PR #155 are merged at the PR #155 Source implementation baseline (merge `cd6bd02fc09a6698d4991dda671131ce03217bcf`): #109～#113 catalogs/snapshots, SourceVersion/processing and association domains, typed ports and single-head `0003_source_evidence`; #121/#124/#126 SQLAlchemy Core tables/mappings, PostgreSQL repositories and specialized UoW/factory; #128/#130 processing application contracts/service/CAS composition; #151/#153 Source-owned association remove/replace contracts/service; and #155 additive immutable no-commit SourceVersion/processing and Task-owned association reads. #81 仍为 OPEN tracking parent，M2 仍为 PARTIAL；registration/intake and exact submitted-input metadata, final RFC-005 Source HTTP/wire DTOs or API/generated client, parser/fragment/retrieval/evidence, Worker, Provider and Web business UI remain unimplemented；`0003_source_evidence` 的 minimal identity/processing/association schema 不是 final intake schema。#82 仍为 OPEN；P-82A～P-86A remain Proposed/pending, and Review lifecycle/schema plus complete outcomes remain unimplemented until those proposals are accepted and typed transaction participants exist
