...
- the real-backend Chromium harness for sufficient/insufficient input, review, Markdown downloads and reload persistence; P4A reconciled its locators against the merged Chinese single-page UI, and P4B ran all three fictional-data cases once with exact cleanup and no real Needs Input/Recovery backend claim;
...
P4B ran once from exact reviewed main `218808040953e8fb0c51eafde23a2266febd7944` in fresh clone `/private/tmp/ai-ecommerce-agent-p4b-I5B2dV`. Offline hydration and non-provider prerequisites passed with Node `24.18.0`, npm `11.16.0`, Python `3.13.14`, uv `0.12.0`, Docker Compose `2.39.1-desktop.1`; package and lock files remained byte-identical. The one retained `PATH=/opt/homebrew/opt/node@24/bin:$PATH ./scripts/mvp0/demo --ephemeral` generated project `ai-ecommerce-agent-mvp0-ephemeral-260823181423-18281-16046` and paired volume `ai-ecommerce-agent-mvp0-ephemeral-260823181423-18281-16046-pg`, reached the Browser URL, and ran the exact real-backend command once from `apps/web`. Three tests passed with one worker: sufficient persistence/reload, confirmation plus both immutable Markdown exports/reload, and insufficient actions unavailable. Exactly one Ctrl-C returned 130; the exact generated containers/network/volume were absent, ports 8000/5173/55432 were free, API/Web processes were gone and the protected default volume metadata was unchanged. The truthful current result is independently reviewed `P4_LOCAL_RELEASE_ACCEPTED`, provider-free and completing the accepted P4 local scope; P5 docs/research is complete and independently reviewed as `P5_REUSE_FROZEN`; direct reuse and platform behavior remain frozen and unauthorized. The closure record is `FINAL_GOAL_REVIEW_PASS`; `MVP0P_GOAL_COMPLETE` becomes current when it reaches `main`, and no successor Goal or next implementation Stage is authorized. The current FastAPI task resource still projects `needsInputRequest: null`, so no real Needs Input/Recovery completion is claimed.
...
DEC-082 supplies the accepted product and frontend baseline. DEC-083 sequences its execution: P0 is complete, Issue #303 / PR #304 is the merged/current P1 shell, Issue #305 / PR #306 is the merged/current P2 Running / Review / Results implementation, and reconciled Issue #247 is the merged/current P3 implementation. Issue #308 P4A is merged/current and Issue #310 P4B records independently reviewed `P4_LOCAL_RELEASE_ACCEPTED`, completing the accepted P4 local scope; P5 docs/research is complete and independently reviewed as `P5_REUSE_FROZEN`, with direct reuse/platform behavior frozen and unauthorized. The closure record is `FINAL_GOAL_REVIEW_PASS`; `MVP0P_GOAL_COMPLETE` becomes current when it reaches `main`, and no successor Goal or next implementation Stage is authorized. Important frontend work uses applicable taste skills; local Kimi Code + Kimi K3 remains a narrow later exact-contract exception, not a P0 call or Luna / Terra fallback. PR #299 is open / unmerged and is not production evidence.
...
- a new user-visible product behavior outside the completed Productization Goal;
...
Final Goal Review: `FINAL_GOAL_REVIEW_PASS` recorded on closure branch; `MVP0P_GOAL_COMPLETE` becomes current when this record reaches `main`; no successor Goal or next implementation Stage authorized
```
