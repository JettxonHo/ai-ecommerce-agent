...
> **Status: predecessor `MVP0P_GOAL_COMPLETE` is historical merge-effective truth after PR #315 · successor [MVP-0L Local AI Web App Delivery Goal](../goals/mvp0-local-ai-web-app-delivery-goal.md) is `ACTIVE` after L0 PR #317 reached `main` · L1 Issue #318 is merged/current through PR #328 after independent five-axis review `PASS` and provider-free runtime acceptance · L2 Issue #329 / PR #330 is merged/current at exact `origin/main@dbccacacc54cb21c393987a8612dfc6aa825093b`; its provider-free runtime, independent five-axis review and fresh Required Checks are `PASS`/`12/12`, and Issue #329 is closed · L3 Issue #331 is the active Docker-only local Web Stage with offline GREEN and one corrected-pin provider-free runtime `PASS` after a historical first image-build `HOLD` on unpublished `uv==0.12.8`; independent five-axis review is `PASS` at `f831519`, fresh Required Checks are `12/12`, and Ready PR #332 is `OPEN`/unmerged. L3 becomes merge-effective/current only when this reviewed record reaches `main`; L4 is gated and not started · predecessor P0–P5 complete · OLD FAST LANE TERMINAL `GOAL_BLOCKED` · P5 `P5_REUSE_FROZEN`**
>
...
- Issue #331's Docker-only local Web lifecycle: the tracked `.command` entry and bounded `scripts/mvp0/local-web` path render the exact `local-web` Compose profile (persistent Postgres, private API, loopback static Web), validate pinned images and host scope, wait for API/Web health before opening the system browser, preserve the default named volume, and pair ephemeral project/volume cleanup. Its first provider-free runtime was a historical image-build-only `HOLD` on unpublished `uv==0.12.8`; no service, health, browser or product-behavior result was produced and guarded cleanup passed. Tests-first `uv==0.12.6` repair preceded one corrected-pin runtime `PASS` with both images, PostgreSQL/API/Web health, browser-after-health, one bounded `/tasks` read showing title “商品上新行动工作台” and heading “行动首页”, Ctrl-C 130 and exact cleanup. Offline RED→GREEN, rendered configuration and static/backend/Web checks pass on the isolated branch. Independent five-axis review is `PASS` at `f831519`, fresh Required Checks are `12/12`, and Ready PR #332 is `OPEN`/unmerged; L3 becomes merge-effective/current only when this reviewed record reaches `main`; L4 is gated and not started. No further runtime or Provider authorization is permitted.
...
P4B ran once from exact reviewed main and passed three fictional-data cases with exact ephemeral cleanup; its truthful predecessor result is independently reviewed `P4_LOCAL_RELEASE_ACCEPTED`, provider-free. Issue #318 is the separate L1 real Needs Input/Recovery acceptance; its exact 6/6 backend and 4/4 browser runtime gates passed and are recorded in [the L1 review](../reviews/mvp0l-l1-needs-input-backend.md), with independent five-axis review `PASS` and PR #328 merged/current. Issue #329 / PR #330 is merged/current at exact base `dbccacacc54cb21c393987a8612dfc6aa825093b` with Issue #329 closed; its one-time provider-free six-test runtime passed in `1.41s` on an isolated ephemeral scope, its independent five-axis review is `PASS`, and fresh Required Checks are `12/12`. Issue #331 is the active L3 Docker-only lifecycle with offline GREEN and the single corrected-pin provider-free runtime `PASS` after the historical first build-only `HOLD`. Independent five-axis review is `PASS` at `f831519`, fresh Required Checks are `12/12`, and Ready PR #332 is `OPEN`/unmerged; L3 becomes merge-effective/current only when this reviewed record reaches `main`; L4 is gated and not started. No further runtime or Provider authorization is permitted. P5 docs/research is independently reviewed as `P5_REUSE_FROZEN`; direct reuse and platform behavior remain frozen and unauthorized. PR #315 made `MVP0P_GOAL_COMPLETE` merge-effective historical truth, and PR #317 made L0 current.
...
DEC-082 supplies the accepted product and frontend baseline. DEC-083's P0–P5 predecessor execution is complete; PR #315 made `MVP0P_GOAL_COMPLETE` current historical truth. DEC-084 and DEC-085 sequence the successor as L0 → L1 → L2 → L3 → L4 → L5 → L6; L0 is active after PR #317, L1 is merged/current through PR #328, and Issue #329 / PR #330 is merged/current at exact base `dbccacacc54cb21c393987a8612dfc6aa825093b` with Issue #329 closed. Issue #331 is the active L3 implementation/review Stage with offline code/test/rendered-Compose `PASS`, a historical first image-build `HOLD` on unpublished `uv==0.12.8`, and one corrected-pin provider-free runtime `PASS`. Independent five-axis review is `PASS` at `f831519`, fresh Required Checks are `12/12`, and Ready PR #332 is `OPEN`/unmerged; L3 becomes merge-effective/current only when this reviewed record reaches `main`; L4 is gated and not started. No further runtime or Provider authorization is permitted. The owner-confirmed MBL-first sequence remains L2 → L3 → L4 → L5 → Agent UI → L6. Important frontend work uses applicable taste skills; local Kimi Code + Kimi K3 remains a narrow later exact-contract exception, not an L0/L2 call or Luna / Terra fallback. PR #299 is open / unmerged and is not production evidence.
...
Successor MVP-0L L3 Docker-only local Web lifecycle: `REVIEW_PASS_PR_OPEN` (Issue #331; isolated branch `codex/mvp0l-l3-docker-local-web`; exact base `dbccacacc54cb21c393987a8612dfc6aa825093b`; 22-path allowlist; first image-build-only `HOLD` on unpublished `uv==0.12.8`; tests-first `uv==0.12.6` repair; one corrected-pin provider-free `--ephemeral` runtime `PASS` with images/health/browser/bounded `/tasks`/Ctrl-C 130/exact cleanup; independent five-axis review `PASS` at `f831519`; fresh Required Checks `12/12`; Ready PR #332 `OPEN`/unmerged; L3 merge-effective/current only when this reviewed record reaches `main`; L4 gated/not started)
