...
> **Pilot boundary:** the Pilot is `ACTIVE` after DEC-087's merge-effective rebaseline. Issue #341's [P0 admission/contract-freeze plan](../product/real-product-to-brief-pilot-p0-plan.md) records the exact P01–P08 cohort, participant, denominator and human/evidence contract. Before that record reaches `main`, P0 is `CONTRACT_FREEZE_PENDING`; once present on `main`, P0 is `P0_CONTRACT_FROZEN` and P1 is `READY_NOT_STARTED`. The exact P0→P6 order and acceptance denominator remain unchanged. Future P1 carries post-confirm/no-export only as a provider-free characterization target, not an approved repair. Every bounded paid execution or cohort still needs a fresh exact-commit owner authorization.
...
Issue #341 is the single P0 planning record. Its [P0 admission/contract-freeze plan](../product/real-product-to-brief-pilot-p0-plan.md) and [Session-012](../sessions/session-012-real-product-to-brief-pilot-p0.md) freeze the exact Owner-selected cohort before observation:
...
The exact seven-path allowlist is `AGENTS.md`, `README.md`, `docs/goals/README.md`, `docs/goals/real-product-to-brief-pilot-goal.md`, `docs/product/real-product-to-brief-pilot-p0-plan.md`, `docs/handoffs/implementation-readiness.md` and `docs/sessions/session-012-real-product-to-brief-pilot-p0.md`. DEC-086, DEC-087, the existing Pilot Contract, Session-010/011, the L5 review, product code and tests remain unchanged. P0 performs no observation, Provider call, participant test, numerator, export or P1/P2 action; `PILOT_EXECUTION_AUTHORIZATION` remains `NOT_AUTHORIZED`.
...
DEC-082 supplies the accepted product and frontend baseline. DEC-083's P0–P5 predecessor execution is complete; PR #315 made `MVP0P_GOAL_COMPLETE` current historical truth. DEC-084/085's L0–L5 evidence remains auditable, while DEC-087 ends the unfinished continuation. MVP-0L is `TERMINAL_INCOMPLETE_L5_FAILED` and the Pilot is `ACTIVE`; Issue #341 carries the merge-durable P0 transition from `CONTRACT_FREEZE_PENDING` / `ADMISSION_PENDING_REVIEW` / `FREEZE_PENDING_REVIEW` to `P0_CONTRACT_FROZEN` / exact denominator 8 / P01–P08 `ADMITTED` once its record reaches `main`. Important frontend work uses applicable taste skills, but Agent UI remains frozen and PR #299 remains open/unmerged non-production evidence.
...
Issue #341 pre-merge: MVP-0L `TERMINAL_INCOMPLETE_L5_FAILED`; Pilot `ACTIVE`; P01–P08 `ADMISSION_PENDING_REVIEW`; denominator `FREEZE_PENDING_REVIEW`; P0 `CONTRACT_FREEZE_PENDING`
Issue #341 merge-effective only: P01–P08 `ADMITTED`; denominator exact 8 frozen; P0 `P0_CONTRACT_FROZEN`; P1 `READY_NOT_STARTED`; `PILOT_EXECUTION_AUTHORIZATION=NOT_AUTHORIZED`
