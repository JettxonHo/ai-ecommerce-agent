...
> **Pilot boundary:** the Pilot is `ACTIVATION_PENDING` and P0 `NOT_STARTED` on this branch. Merge makes Pilot `ACTIVE` and P0 only `READY_NOT_STARTED`; no cohort, real input, denominator or business evidence has been registered. The exact P0→P6 order and acceptance denominator remain unchanged. Future P1 carries post-confirm/no-export only as a provider-free characterization target, not an approved repair. Every bounded paid execution or cohort still needs a fresh exact-commit owner authorization.
...
DEC-082 supplies the accepted product and frontend baseline. DEC-083's P0–P5 predecessor execution is complete; PR #315 made `MVP0P_GOAL_COMPLETE` current historical truth. DEC-084/085's L0–L5 evidence remains auditable, while DEC-087 ends the unfinished continuation. On this branch MVP-0L is `REBASELINE_PENDING`, Pilot `ACTIVATION_PENDING`, P0 `NOT_STARTED`; only after merge do MVP-0L `TERMINAL_INCOMPLETE_L5_FAILED`, Pilot `ACTIVE` and P0 `READY_NOT_STARTED` become effective. Important frontend work uses applicable taste skills, but Agent UI remains frozen and PR #299 remains open/unmerged non-production evidence.
...
Issue #339 branch: MVP-0L `REBASELINE_PENDING`; Pilot `ACTIVATION_PENDING`; P0 `NOT_STARTED`
Issue #339 merge-effective only: MVP-0L `TERMINAL_INCOMPLETE_L5_FAILED`; Pilot `ACTIVE`; P0 `READY_NOT_STARTED`
