...
- **本地 Action Workbench（DEC-082）：** `/tasks` 以创建 Task 为主要动作，并从 Needs Input / Review / Failed-Recoverable / 最近活跃中只突出一个恢复项；稳定 Task 内用包含用户可见 Header 的中文五阶段进度、一个 Active Workspace 与 `320–360px` 可折叠 Context Rail 呈现结构化 Review、行动摘要、Marketing / Xiaohongshu Results 和技术细节。视觉为中文优先“运营编辑部 / 策略桌”，不采用 Dashboard 或 chat-first 产品形态。
