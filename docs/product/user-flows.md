...
> 当前已确认：MVP 核心任务的高层级流程（DEC-003）、首个演示场景映射（DEC-004）、输入分层（DEC-005）、四层输出（DEC-006）、单一关键审核节点 + 异常暂停（DEC-007）、证据与可追溯（DEC-008）、阶段级失效（DEC-009）、单任务工作台与确认式局部重跑（DEC-044），以及最小输入、文件限制与冲突分级（DEC-045）；详细控件、引用 UI、公共状态与工作流技术实现仍未确认。
...
- 已确认高层任务流程、平台映射、输入分层、四层输出、Human Review、证据追溯、工作台、最小输入、文件限制、冲突分级、确认式局部重跑和行为型演示成功边界（DEC-003～009 / DEC-042 / DEC-044 / DEC-045）；具体控件、引用 UI、公共状态与工作流实现仍待讨论。
