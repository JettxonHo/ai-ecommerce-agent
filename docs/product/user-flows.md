...
- **跨会话进入与行动首页（DEC-062 / 082）：** 用户从 `/tasks` 创建新任务、恢复一个主要进行中任务或查看少量最近任务；任务摘要显示名称 / 临时名称、品类、当前阶段或等待状态、最近更新时间和主要下一步动作。选择后进入稳定 Task 深链，并继续使用同一个 TaskWorkbench。`/tasks` 不提供图表、搜索、高级筛选、批量、归档或 Dashboard 模块。
...
> **DEC-082 具体布局：** Task 深链使用“资料整理 → 用户洞察 → 商品定位 → 营销 Brief → 小红书 Brief”五阶段中文轨道；一个 Active Workspace 展示当前主要动作，证据 / 来源 / 限制 / 风险进入展开宽度 `320–360px` 的可折叠 Context Rail。Review 默认呈现结构化语义组和确认动作；Results 分别呈现 Marketing / Xiaohongshu 结果、版本 / 限制与导出动作；raw JSON 仅在“技术细节”后按需查看。AI 以当前阶段、处理状态、依据和下一步表达，不提供聊天式主流程。
