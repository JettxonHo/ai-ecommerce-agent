...
- **最低可运行输入：** 商品名称、商品品类、商品功能 / 参数 / 文字说明、价格或价格区间、商家主要卖点、商品图片或其他展示资料（图片或足以描述商品的文字二选一）、本次推广目标。
...
> 注：以上为输入**分层原则与字段范围**；**未**确认每个字段的具体数据类型、最低字段是否全部强制、商品图片是否技术必填、文件格式、商品链接与自动抓取、评论 / 竞品自动采集、联网搜索、长期知识库、向量数据库、文档切分与检索、完整度评分算法、信息不足时具体询问内容、隐私 / 权限 / 数据保存策略。
...
> 注：以上为输出**四层主结构、追溯原则与「小红书完整笔记非核心交付物」**；**未**确认各层最终字段、哪些字段必填、输出格式（Markdown / JSON / 表格 / 其他）、引用显示形式、置信度计算方式、是否将完整小红书笔记作为附加能力、用户能否单独编辑某一层、人工审核节点、Agent / RAG / Skill 技术实现、数据库存储方式。
...
> 注：以上为**人机协作原则**（单一关键审核节点 + 异常暂停 + 用户最终判断权）；**未**确认是否采用 LangGraph / Interrupt API、数据库级持久化 / Checkpoint 实现方式、用户修改后重跑全部还是局部流程、审核页面结构、草稿编辑粒度、异常判断规则、高风险表达范围、多人协作审核、版本历史、能否跳过常规审核节点。本决定有潜在架构影响，当前不建 Architecture Decision / RFC。
...
> 注：以上为**可靠性原则与五类标记**；**未**确认引用 UI、是否显示置信度数值、置信度算法、来源 ID 与数据结构、是否使用 RAG / 向量数据库 / 网页检索 / 知识图谱、是否保存原始文档片段、全量或局部重跑、LangGraph State / Checkpoint、数据库存储方式、外部来源可信度评估规则。**可靠性原则已成为后续架构设计与开源仓库选型的硬约束**，当前不建 RFC。
...
> 注：以上为**阶段级失效与局部重跑规则**；**未**确认 LangGraph / Checkpoint / Interrupt / 状态数据库 / 自动重跑交互 / 版本历史 / 字段级依赖图 / 前端界面 / Agent 或节点数量。与 DEC-007 / DEC-008 同为后续架构 RFC 的业务约束，当前不建 RFC。
...
- 输出可靠性 —— **五类结论标记 + 可追溯原则已确认**（DEC-008）；引用 UI / 置信度算法 / 来源数据结构 / RAG / 向量库 / 知识图谱待确认
- 失效与重跑 —— **阶段级失效 + 局部重跑规则已确认**（DEC-009）；字段级依赖图暂缓；LangGraph / Checkpoint / 自动重跑交互 / 版本历史待确认
...
- 输入设计：**分层原则已确认**（DEC-005）；字段数据类型、文件格式、图片是否技术必填、商品链接 / 自动抓取、评论 / 竞品自动采集、联网搜索、长期知识库 / 向量库、完整度评分与交互询问内容待确认。
...
- 输出可靠性 / 可追溯：**原则层已确认**（DEC-008：五类结论标记 + 七条可靠性原则）；引用 UI / 置信度算法 / 来源数据结构 / RAG / 向量库 / 知识图谱仍待确认。
- 失效与局部重跑：**规则层已确认**（DEC-009：阶段级失效 + 局部重跑 + 重要 / 非重要修改区分）；LangGraph / Checkpoint / 自动重跑交互 / 版本历史 / 字段级依赖图待确认。
- 完整小红书标题 / 正文是否作为附加能力：**尚未确认**（DEC-006 已确认其不作为 MVP 核心交付物）。
- 产品价值与评估指标（Question-003）：**框架层已确认**（DEC-010：三维评价 + 六项优先指标，不以流畅度 / 销量为唯一标准）；指标公式 / 阈值 / 测试集 / 人数 / 埋点 / Dashboard 待确认。
- Agent、RAG、Skill 在产品中各自承担什么角色：**尚未确认**。
